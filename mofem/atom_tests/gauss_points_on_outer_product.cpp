...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    auto test_quad = [&]() {
      ...
      if (fabs(20.0 - sum_coords) > eps) {
        SETERRQ1(PETSC_COMM_SELF, MOFEM_DATA_INCONSISTENCY,
                 "wrong result %3.4e", 20.0 - sum_coords);
      }
      if (fabs(1.0 - sum_gauss_pts) > eps) {
        SETERRQ1(PETSC_COMM_SELF, MOFEM_DATA_INCONSISTENCY,
                 "wrong result %3.4e", 1.0 - sum_gauss_pts);
      }
    ...
    auto test_hex = [&]() {
      ...
      double eps = 1e-8;
      if (fabs(52.0 - sum_coords) > eps) {
        SETERRQ1(PETSC_COMM_SELF, MOFEM_DATA_INCONSISTENCY,
                 "wrong result %3.4e", 52 - sum_coords);
      }
      if (fabs(1.0 - sum_gauss_pts) > eps) {
        SETERRQ1(PETSC_COMM_SELF, MOFEM_DATA_INCONSISTENCY,
                 "wrong result %3.4e", 1 - sum_gauss_pts);
      }
      MoFEMFunctionReturn(0);
    };
