...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    ierr = m_field.query_interface<Tools>()->getEntitiesByTypeAndRefLevel(bit_level1,BitRefLevel().set(),MBTET,out_meshset_tet); CHKERRQ(ierr);
    Range tets;
    ...
    // Range children_new;
    // ierr = m_field.query_interface<Tools>()->getEntitiesByRefLevel(bit_level1,bit_level1,children_new); CHKERRQ(ierr);
