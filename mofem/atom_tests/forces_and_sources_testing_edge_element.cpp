...
int main(int argc, char *argv[]) {
  ...
  ierr = m_field.set_field_order(root_set,MBEDGE,"MESH_NODE_POSITIONS",1); CHKERRG(ierr);
  ierr = m_field.set_field_order(root_set,MBTRI,"MESH_NODE_POSITIONS",1); CHKERRG(ierr);
  ierr = m_field.set_field_order(root_set,MBTET,"MESH_NODE_POSITIONS",1); CHKERRG(ierr);
  ...
  rval = moab.get_adjacencies(tets_skin,1,false,tets_skin_edges,moab::Interface::UNION); CHKERRG(rval);
  ierr = m_field.add_ents_to_finite_element_by_type(tets_skin_edges,MBEDGE,"TEST_FE"); CHKERRG(ierr);
