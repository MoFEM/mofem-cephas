...
int main(int argc, char *argv[]) {
  ...
  ierr = m_field.add_field("FIELD2",NOFIELD,NOBASE,3); CHKERRG(ierr);
  ...
  ierr = m_field.add_ents_to_finite_element_by_type(tets_skin,MBTRI,"TEST_FE1"); CHKERRG(ierr);
  ierr = m_field.add_ents_to_finite_element_by_type(tets_skin,MBTRI,"TEST_FE2"); CHKERRG(ierr);
  ...
  struct MyOp1: public FaceElementForcesAndSourcesCore::UserDataOperator {
    ...
    MoFEMErrorCode doWork(
      ...
      ierr = getProblemColIndices("FIELD1",col_type,col_side,col_indices); CHKERRG(ierr);
