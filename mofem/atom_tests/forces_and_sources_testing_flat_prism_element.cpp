...
int main(int argc, char *argv[]) {
  ...
  ierr = m_field.add_ents_to_finite_element_by_type(root_set,MBPRISM,"TEST_FE1",10); CHKERRQ(ierr);
  ierr = m_field.add_ents_to_finite_element_by_type(root_set,MBPRISM,"TEST_FE2",10); CHKERRQ(ierr);
  ...
  struct MyOp: public FlatPrismElementForcesAndSourcesCore::UserDataOperator {
    ...
    MoFEMErrorCode doWork(
      ...
      for(
        DoubleAllocator::iterator it = getNormalsAtGaussPtF3().data().begin();
        it!=getNormalsAtGaussPtF3().data().end();it++
      ) {
        *it = fabs(*it)<eps ? 0.0 : *it;
      }
      for(
        DoubleAllocator::iterator it = getTangent1AtGaussPtF3().data().begin();
        it!=getTangent1AtGaussPtF3().data().end();it++
      ) {
        *it = fabs(*it)<eps ? 0.0 : *it;
      }
      for(
        DoubleAllocator::iterator it = getTangent2AtGaussPtF3().data().begin();
        it!=getTangent2AtGaussPtF3().data().end();it++
      ) {
        *it = fabs(*it)<eps ? 0.0 : *it;
      }
      for(
        DoubleAllocator::iterator it = getNormalsAtGaussPtF4().data().begin();
        it!=getNormalsAtGaussPtF4().data().end();it++
      ) {
        *it = fabs(*it)<eps ? 0.0 : *it;
      }
      for(
        DoubleAllocator::iterator it = getTangent1AtGaussPtF4().data().begin();
        it!=getTangent1AtGaussPtF4().data().end();it++
      ) {
        *it = fabs(*it)<eps ? 0.0 : *it;
      }
      for(
        DoubleAllocator::iterator it = getTangent2AtGaussPtF4().data().begin();
