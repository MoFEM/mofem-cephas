/**
 * @file HookeOps.hpp
 * @brief Implementation of Hookes operator Hookes for linear elasticit problems in MoFEM.
 * @version 0.14.0
 * @date 2025-04-10
 */
...
// Assemble domain right-hand-side vector- internal force vector; 'is_non_linear' choosen false for solving  linear problems by default
...
MoFEMErrorCode opFactoryDomainRhs(
    MoFEM::Interface &m_field,
    boost::ptr_deque<ForcesAndSourcesCore::UserDataOperator> &pip,
    std::string field_name, std::string block_name, Sev sev, bool is_non_linear = false, double scale = 1 ) {
  MoFEMFunctionBegin;
  ...
  CHKERR opFactoryDomainRhs<DIM, A, I, DomainEleOp>(m_field, pip, field_name,
                                                    common_ptr, sev, is_non_linear);

  MoFEMFunctionReturn(0);
}
// Assemble domain LHS K factory
template <int DIM, AssemblyType A, IntegrationType I, typename DomainEleOp>
...
// Overloaded LHS factory for scaling
template <int DIM, AssemblyType A, IntegrationType I, typename DomainEleOp>
MoFEMErrorCode opFactoryDomainLhs(
    MoFEM::Interface &m_field,
    boost::ptr_deque<ForcesAndSourcesCore::UserDataOperator> &pip,
