...
struct MyTransport: public MixTransportElement {
  ...
  PetscErrorCode addBoundaryElements(BitRefLevel &ref_level) {
    MoFEMFunctionBeginHot;
    Range tets;
    ierr = mField.query_interface<Tools>()->getEntitiesByTypeAndRefLevel(ref_level,BitRefLevel().set(),MBTET,tets);
    ...
    Range bit_tris;
    ierr = mField.query_interface<Tools>()->getEntitiesByTypeAndRefLevel(ref_level,BitRefLevel().set(),MBTRI,bit_tris);
