...
int main(int argc, char *argv[]) {
  ...
  // Assemble Aij and F. Define Dirichlet bc element, which stets constrains to MatrixDouble
  ...
  if(!flag_cubit_disp){
    dirichlet_bc_ptr = boost::shared_ptr<FEMethod>(new
  ...
  // That sets Dirichlet bc objects that problem is linear, i.e. no newton (SNES) solver
  ...
  ierr = DMoFEMPreProcessFiniteElements(dm, dirichlet_bc_ptr.get()); CHKERRQ(ierr);
