...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    if(flg_list_of_sidesets != PETSC_TRUE) {
      SETERRQ(PETSC_COMM_SELF,MOFEM_INVALID_DATA,"List of sidesets not given -my_side_sets ...");
    }
    ...
    for(
      ...
      meshsets_index.get<CubitMeshSets_mask_meshset_mi_tag>().lower_bound(SIDESET);
      mit!=meshsets_index.get<CubitMeshSets_mask_meshset_mi_tag>().upper_bound(SIDESET);
      mit++
    ) {
      std::cout << "Sidesset on the mesh id = " << mit->getMeshsetId() << std::endl;
    }

    for(int mm = 0;mm!=nb_sidesets;mm++) {
      CubitMeshSet_multiIndex::index<Composite_Cubit_msId_And_MeshSetType_mi_tag>::type::iterator mit;
      mit = meshsets_index.get<Composite_Cubit_msId_And_MeshSetType_mi_tag>().find(boost::make_tuple(sidesets[mm],SIDESET));
      if(mit==meshsets_index.get<Composite_Cubit_msId_And_MeshSetType_mi_tag>().end()) {
        SETERRQ1(PETSC_COMM_SELF,MOFEM_INVALID_DATA,"No sideset in database id = %d",sidesets[mm]);
      }
      ierr = PetscPrintf(PETSC_COMM_WORLD,"Split sideset %d\n",mit->getMeshsetId()); CHKERRQ(ierr);
      EntityHandle cubit_meshset = mit->getMeshset();
      {
        ...
        // split faces and
        ierr = interface_ptr->splitSides(ref_level_meshset,bit_levels.back(),cubit_meshset,false,true,0); CHKERRQ(ierr);
        // clean meshsets
    ...
    if(squash_bit_levels==PETSC_TRUE) {
      for(int ll  = 0;ll!=bit_levels.size()-1;ll++) {
        ierr = m_field.delete_ents_by_bit_ref(bit_levels[ll],bit_levels[ll],true); CHKERRQ(ierr);
      }
      ierr = m_field.shift_right_bit_ref(bit_levels.size()-1); CHKERRQ(ierr);
    }
