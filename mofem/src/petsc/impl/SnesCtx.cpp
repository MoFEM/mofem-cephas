...
PetscErrorCode SnesRhs(SNES snes, Vec x, Vec f, void *ctx) {
  ...
  CHKERRG(ierr);
  ierr = VecGhostUpdateBegin(f, INSERT_VALUES, SCATTER_FORWARD);
  CHKERRG(ierr);
  ierr = VecGhostUpdateEnd(f, INSERT_VALUES, SCATTER_FORWARD);
  CHKERRG(ierr);
  SnesCtx::BasicMethodsSequence::iterator bit =
      snes_ctx->preProcess_Rhs.begin();
  for (; bit != snes_ctx->preProcess_Rhs.end(); bit++) {
    ierr = (*bit)->setSnes(snes);
    CHKERRG(ierr);
    (*bit)->snes_x = x;
    (*bit)->snes_f = f;
    ierr = (*bit)->setSnesCtx(SnesMethod::CTX_SNESSETFUNCTION);
    CHKERRG(ierr);
    ierr = snes_ctx->mField.problem_basic_method_preProcess(
        snes_ctx->problemName, *(*(bit)));
    CHKERRG(ierr);
    ierr = (*bit)->setSnesCtx(SnesMethod::CTX_SNESNONE);
    CHKERRG(ierr);
  }
  SnesCtx::FEMethodsSequence::iterator lit = snes_ctx->loops_to_do_Rhs.begin();
  for (; lit != snes_ctx->loops_to_do_Rhs.end(); lit++) {
    ierr = lit->second->setSnesCtx(SnesMethod::CTX_SNESSETFUNCTION);
    CHKERRG(ierr);
    ierr = lit->second->setSnes(snes);
    CHKERRG(ierr);
    lit->second->snes_x = x;
    lit->second->snes_f = f;
    // PetscSynchronizedPrintf(PETSC_COMM_WORLD,"\t\tLoop FE for Rhs:
    // %s\n",lit->first.c_str());  PetscSynchronizedFlush(PETSC_COMM_WORLD);
    ierr = snes_ctx->mField.loop_finite_elements(
  ...
  for (; bit != snes_ctx->postProcess_Rhs.end(); bit++) {
    ...
    ierr = (*bit)->setSnesCtx(SnesMethod::CTX_SNESSETFUNCTION);
    CHKERRG(ierr);
    ierr = snes_ctx->mField.problem_basic_method_postProcess(
        snes_ctx->problemName, *(*(bit)));
    CHKERRG(ierr);
    ierr = (*bit)->setSnesCtx(SnesMethod::CTX_SNESNONE);
    CHKERRG(ierr);
...
PetscErrorCode SnesMat(SNES snes, Vec x, Mat A, Mat B, void *ctx) {
  ...
  for (; bit != snes_ctx->preProcess_Mat.end(); bit++) {
    ...
    (*bit)->snes_A = A;
    (*bit)->snes_B = B;
    ierr = (*bit)->setSnesCtx(SnesMethod::CTX_SNESSETJACOBIAN);
    CHKERRG(ierr);
    ierr = snes_ctx->mField.problem_basic_method_preProcess(
        snes_ctx->problemName, *(*(bit)));
    CHKERRG(ierr);
  ...
  for (; lit != snes_ctx->loops_to_do_Mat.end(); lit++) {
    ierr = lit->second->setSnesCtx(SnesMethod::CTX_SNESSETJACOBIAN);
    CHKERRG(ierr);
    ierr = lit->second->setSnes(snes);
    CHKERRG(ierr);
    lit->second->snes_x = x;
  ...
  ierr = MatAssemblyBegin(B, snes_ctx->typeOfAssembly);
  CHKERRG(ierr);
  ierr = MatAssemblyEnd(B, snes_ctx->typeOfAssembly);
  CHKERRG(ierr);
