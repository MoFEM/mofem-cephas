...
MoFEMErrorCode BitLevelCoupler::buildAdjacenciesVerticesOnTets(
  ...
  for (; it != hi_it; it++) {
    ...
    const auto ref_parent_ent = getRefEntityPtr(m_field, parent_ent);
...
MoFEMErrorCode BitLevelCoupler::buildAdjacenciesEdgesFacesVolumes(
  ...
  for (; eit != hi_eit; eit++) {
    ...
    for (int nn = 0; nn < num_nodes; nn++) {
      conn_parents[nn] = getRefEntityPtr(m_field, conn[nn])->getParentEnt();
  ...
  for (auto fit = fields_ptr->begin(); fit != fields_ptr->end(); fit++) {
    ...
    if (verify) {
      ...
      for (; pit != parents.end();
        ...
        // ref ent pointers
        auto ref_parent_ptr = getRefEntityPtr(m_field,  *pit);
        auto ref_child_ptr = getRefEntityPtr(m_field, *cit);
...
MoFEMErrorCode BitLevelCoupler::copyFieldDataFromParentToChildren(
  ...
  for (Range::iterator eit = ents.begin(); eit != ents.end(); eit++) {
    const auto ref_ent = getRefEntityPtr(m_field, *eit);
