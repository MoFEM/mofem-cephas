...
MoFEMErrorCode BitRefManager::setBitRefLevel(const Range &ents,
  ...
  MOFEM_LOG_FUNCTION();
  MOFEM_LOG_C("BitRefSelf", Sev::noisy, "nb. entities to add %d",
              ents.size());
...
MoFEMErrorCode BitRefManager::addBitRefLevelByDim(const EntityHandle meshset,
  ...
  if (verb == VERY_NOISY) {
    MOFEM_LOG_FUNCTION();
    MOFEM_LOG("BitRefSelf", Sev::noisy) << "Add add bit ref level by dim " << ents;
  }
...
MoFEMErrorCode BitRefManager::getAdjacencies(
  ...
  for (Range::iterator eit = adj_entities.begin(); eit != adj_entities.end();
    ...
    if (verb > VERBOSE) {
        ...
        MOFEM_LOG_FUNCTION();
        MOFEM_LOG("BitRef", Sev::noisy) << "to: " << adj_entity.getBitRefLevel()
                                        << " : " << adj_entity;
    }
