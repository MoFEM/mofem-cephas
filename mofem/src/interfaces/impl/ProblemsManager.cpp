...
MoFEMErrorCode ProblemsManager::partitionMesh(const Range &ents, const int dim,
                                              const int adj_dim,
                                              const int n_parts,
                                              Tag *th_vertex_weights,
                                              Tag *th_edge_weights,
                                              Tag *th_part_weights, int verb) {
  MoFEM::Interface &m_field = cOre;
  MoFEMFunctionBeginHot;
  ...
  int rstart, rend, nb_elems;
  {
    ...
    CHKERRG(ierr);
    ierr = PetscLayoutSetSize(layout, ents.size());
    CHKERRG(ierr);
    ierr = PetscLayoutSetUp(layout);
    CHKERRG(ierr);
    ierr = PetscLayoutGetSize(layout, &nb_elems);
    CHKERRG(ierr);
    ierr = PetscLayoutGetRange(layout, &rstart, &rend);
    CHKERRG(ierr);
  ...
  int *_j;
  {
    ...
    std::vector<int> i(rend - rstart + 1, 0), j;
    {
      ...
      for (int ii = 0; fe_it != ents.end(); fe_it++, ii++) {
        ...
        if (m_field.get_moab().type_from_handle(*fe_it) == MBENTITYSET) {
          ...
          Range adj_ents;
          rval = mtu.get_bridge_adjacencies(*fe_it, adj_dim, dim, adj_ents);
          CHKERRQ_MOAB(rval);
    ...
    ierr = PetscMalloc(i.size() * sizeof(int), &_i);
    CHKERRG(ierr);
    ierr = PetscMalloc(j.size() * sizeof(int), &_j);
    CHKERRG(ierr);
  ...
  if (th_vertex_weights != NULL) {
    ierr = PetscMalloc(weigth_ents.size() * sizeof(int), &vertex_weights);
    CHKERRG(ierr);
    rval = m_field.get_moab().tag_get_data(*th_vertex_weights, weigth_ents,
                                           vertex_weights);
    CHKERRQ_MOAB(rval);
  }

  {
    ...
    ierr = MatCreateMPIAdj(m_field.get_comm(), rend - rstart, nb_elems, _i, _j,
                           PETSC_NULL, &Adj);
    CHKERRG(ierr);
    ierr = MatSetOption(Adj, MAT_STRUCTURALLY_SYMMETRIC, PETSC_TRUE);
    CHKERRG(ierr);
    ...
    CHKERRG(ierr);
    if (th_vertex_weights != NULL) {
      ierr = MatPartitioningSetVertexWeights(part, vertex_weights);
      CHKERRG(ierr);
    }
    PetscBool same;
    ...
    if (same) {
#ifdef PARMETIS
      ierr = MatPartitioningApply_Parmetis_MoFEM(part, &is);
      CHKERRG(ierr);
#endif
    } else {
    ...
    CHKERRG(ierr);
    ierr = ISGetIndices(is_gather_num, &gids);
    CHKERRG(ierr);
    ...
    Tag part_tag = pcomm->part_tag();
    {
      ...
      rval = m_field.get_moab().tag_set_data(part_tag, ents, part_number);
      CHKERRQ_MOAB(rval);
    ...
    std::map<int, Range> parts_ents;
    {
      ...
      //   rval = m_field.get_moab().delete_entities(tagged_sets);
      //   CHKERRQ_MOAB(rval); tagged_sets.clear();
      // }
      if (!tagged_sets.empty()) {
        rval = m_field.get_moab().tag_delete_data(part_tag, tagged_sets);
        CHKERRQ_MOAB(rval);
      }
      if (n_parts > (int)tagged_sets.size()) {
        ...
        for (int i = 0; i < num_new; i++) {
          ...
          rval = m_field.get_moab().create_meshset(
        ...
        for (int i = 0; i < num_del; i++) {
          ...
          rval = m_field.get_moab().delete_entities(&old_set, 1);
          CHKERRQ_MOAB(rval);
        }
      }
      ...
      for (int i = 0; i < n_parts; i++)
        dum_ids[i] = i;
      rval = m_field.get_moab().tag_set_data(part_tag, tagged_sets,
                                             &*dum_ids.begin());
      CHKERRQ_MOAB(rval);
      rval = m_field.get_moab().clear_meshset(tagged_sets);
      CHKERRQ_MOAB(rval);
      ...
      for (int pp = 0; pp != n_parts; pp++) {
        ...
        for (int dd = dim - 1; dd != -1; dd--) {
          ...
          if (dim > 0) {
            ...
            rval =
                m_field.get_moab().get_connectivity(dim_ents, adj_ents, true);
            CHKERRQ_MOAB(rval);
          }
      ...
      if (verb > 1) {
        for (int rr = 0; rr != m_field.get_comm_size(); rr++) {
          ...
          CHKERRQ_MOAB(rval);
          rval = m_field.get_moab().add_entities(meshset, parts_ents[rr]);
          CHKERRQ_MOAB(rval);
      ...
      for (int pp = 0; pp != n_parts; pp++) {
        rval = m_field.get_moab().add_entities(tagged_sets[pp], parts_ents[pp]);
        CHKERRQ_MOAB(rval);
      }
      // for(int rr = 0;rr!=m_field.get_comm_size();rr++) {
      //   ostringstream ss;
      ...
      for (int dd = 0; dd <= dim; dd++) {
        ...
        for (int pp = 0; pp != n_parts; pp++) {
          ...
          for (Range::iterator eit = dim_ents.begin(); eit != dim_ents.end();
               eit++) {
            if (dd > 0) {
              rval = m_field.get_moab().tag_set_data(part_tag, &*eit, 1, &pp);
              CHKERRQ_MOAB(rval);
            }
            rval = m_field.get_moab().tag_set_data(gid_tag, &*eit, 1, &gid);
            CHKERRQ_MOAB(rval);
            gid++;
          }
        }
    ...
    CHKERRG(ierr);
    ierr = ISRestoreIndices(is_gather_num, &gids);
    CHKERRG(ierr);
...
MoFEMErrorCode ProblemsManager::buildProblemOnDistributedMesh(
  ...
  ierr = m_field.get_problem(name, &problem_ptr);
  CHKERRG(ierr);
  ierr = buildProblemOnDistributedMesh(const_cast<Problem *>(problem_ptr),
                                       square_matrix, verb);
  CHKERRG(ierr);
...
MoFEMErrorCode ProblemsManager::buildSubProblem(
  ...
  for (int ss = 0; ss != (square_matrix ? 1 : 2); ss++) {
    ...
    // Set global indexes, compress global indices
    {
      ...
      IS is;
      ierr = ISCreateGeneral(m_field.get_comm(), nb, &*main_indices.begin(),
      ...
      if (ss == 0) {
        ...
        ierr = PetscObjectReference((PetscObject)ao);
        CHKERRG(ierr);
      } else {
      ...
      ierr = AOApplicationToPetscIS(ao, is);
      CHKERRG(ierr);
      ...
      // set global indices to nodes not on this part
      {
        ...
        IS is;
        ierr = ISCreateGeneral(m_field.get_comm(), nb,
  ...
  if (square_matrix) {
    ...
    ierr =
        PetscObjectReference((PetscObject)out_problem_it->getSubData()->rowIs);
    CHKERRG(ierr);
    ierr =
        PetscObjectReference((PetscObject)out_problem_it->getSubData()->rowMap);
    CHKERRG(ierr);
  }
...
MoFEMErrorCode ProblemsManager::partitionGhostDofs(const std::string &name,
  ...
  if (m_field.get_comm_size() > 1) {
    NumeredDofEntity_multiIndex_uid_view_ordered ghost_idx_col_view,
        ghost_idx_row_view;
    // intearte finite elements on this partotion
    NumeredEntFiniteElement_multiIndex::index<Part_mi_tag>::type::iterator
        fe_it,
        hi_fe_it;
    // get elements on this partition
    fe_it = p_miit->numeredFiniteElements.get<Part_mi_tag>().lower_bound(
        m_field.get_comm_rank());
    hi_fe_it = p_miit->numeredFiniteElements.get<Part_mi_tag>().upper_bound(
        m_field.get_comm_rank());
    ...
    // set local indices
    for (int ss = 0; ss < loop_size; ss++) {
      NumeredDofEntity_multiIndex_uid_view_ordered::iterator ghost_idx_miit =
          ghost_idx_view[ss]->begin();
