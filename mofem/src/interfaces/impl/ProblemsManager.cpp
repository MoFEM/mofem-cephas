...
namespace MoFEM {
  ...
  PetscErrorCode ProblemsManager::buildProblem(Problem *problem_ptr,const bool square_matrix,int verb) {
    ...
    if(verb>0) {
      ...
      problem_ptr->numeredDofsRows->size(),problem_ptr->numeredDofsCols->size());
    }
    if(verb>1) {
      EntFiniteElement_multiIndex::iterator miit = fe_ent_ptr->begin();
      EntFiniteElement_multiIndex::iterator hi_miit = fe_ent_ptr->end();
      ...
      NumeredDofEntity_multiIndex::iterator miit_dd_row = problem_ptr->numeredDofsRows->begin();
      ...
      NumeredDofEntity_multiIndex::iterator miit_dd_col = problem_ptr->numeredDofsCols->begin();
  ...
  PetscErrorCode ProblemsManager::buildProblemOnDistributedMesh(
    ...
    if(square_matrix) {
      ...
      problem_ptr->numeredDofsCols = boost::shared_ptr<NumeredDofEntity_multiIndex>(
  ...
  PetscErrorCode ProblemsManager::buildSubProblem(
    ...
    boost::shared_ptr<NumeredDofEntity_multiIndex> main_problem_dofs[] = {
      main_problem_it->numeredDofsRows,
      main_problem_it->numeredDofsCols
    };
   ...
   PetscErrorCode ProblemsManager::buildCompsedProblem(
     ...
     for(int ss = 0;ss!=((square_matrix)?1:2);ss++) {
      ...
      for(unsigned int pp = 0;pp!=add_prb_ptr[ss]->size();pp++) {
        ...
        if(ss == 0) {
          ...
          dit = (*add_prb_ptr[ss])[pp]->numeredDofsCols->get<PetscGlobalIdx_mi_tag>().begin();
          hi_dit = (*add_prb_ptr[ss])[pp]->numeredDofsCols->get<PetscGlobalIdx_mi_tag>().end();
        }
        int is_nb = 0;
        for(;dit!=hi_dit;dit++) {
          // cerr << **dit << endl;
    ...
    for(int ss = 0;ss!=((square_matrix)?1:2);ss++) {
      ...
      if(ss==0) {
        out_problem_it->numeredDofsRows->insert(dofs_shared_array.begin(),dofs_shared_array.end());
      } else {
        out_problem_it->numeredDofsCols->insert(dofs_shared_array.begin(),dofs_shared_array.end());
      }
    }
  ...
  PetscErrorCode ProblemsManager::partitionSimpleProblem(const std::string &name,int verb) {
    ...
    NumeredDofEntitysByIdx &dofs_row_by_idx = p_miit->numeredDofsRows->get<Idx_mi_tag>();
  ...
  PetscErrorCode ProblemsManager::partitionProblem(const std::string &name,int verb) {
    ...
    try {
      ...
      if(square_matrix) {
        ...
        const_cast<NumeredDofEntitysByIdx&>(p_miit->numeredDofsCols->get<Idx_mi_tag>());
  ...
  PetscErrorCode ProblemsManager::inheretPartition(
    ...
    const boost::shared_ptr<NumeredDofEntity_multiIndex> dofs_row = p_miit_row->numeredDofsRows;
    ...
    boost::shared_ptr<NumeredDofEntity_multiIndex> dofs_col = p_miit_col->numeredDofsCols;
  ...
  PetscErrorCode ProblemsManager::partitionGhostDofs(const std::string &name,int verb) {
    ...
    if(verb>0) {
      ...
      if(verb>1) {
        NumeredDofEntity_multiIndex::iterator miit_dd_col = p_miit->numeredDofsCols->begin();
  ...
  PetscErrorCode ProblemsManager::partitionGhostDofsOnDistributedMesh(const std::string &name,int verb) {
    ...
    if(m_field.get_comm_size()>1) {
      ...
      for(int ss = 0;ss!=loop_size;ss++) {
        ...
        if(ss == 0) {
          dit = p_miit->numeredDofsRows->get<PetscLocalIdx_mi_tag>().lower_bound(-1);
          hi_dit = p_miit->numeredDofsRows->get<PetscLocalIdx_mi_tag>().upper_bound(-1);
        } else {
          dit = p_miit->numeredDofsCols->get<PetscLocalIdx_mi_tag>().lower_bound(-1);
          hi_dit = p_miit->numeredDofsCols->get<PetscLocalIdx_mi_tag>().upper_bound(-1);
        }
        ...
        NumeredDofEntity_multiIndex_uid_view_ordered::iterator gdit = ghost_idx_view.begin();
        for(;gdit!=ghost_idx_view.end();gdit++) {
          if(gdit->get()->getPStatus() == 0) continue;
          ...
          NumeredDofEntityByUId::iterator diit = numered_dofs_ptr.lock()->find((*gdit)->getGlobalUniqueId());
          if(diit->get()->getPetscGlobalDofIdx()==-1) {
            SETERRQ(PETSC_COMM_WORLD,MOFEM_DATA_INCONSISTENCY,"data inconsitency");
          }
