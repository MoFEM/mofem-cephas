...
MoFEMErrorCode MeshsetsManager::getTags(int verb) {
  ...
  moab::Interface &moab = m_field.get_moab();
  int default_val = -1;
  rval = moab.tag_get_handle(DIRICHLET_SET_TAG_NAME, 1, MB_TYPE_INTEGER, nsTag,
                             MB_TAG_SPARSE | MB_TAG_CREAT, &default_val);
  CHKERRQ_MOAB(rval);
  rval = moab.tag_get_handle(NEUMANN_SET_TAG_NAME, 1, MB_TYPE_INTEGER, ssTag,
                             MB_TAG_SPARSE | MB_TAG_CREAT, &default_val);
  CHKERRQ_MOAB(rval);
  ...
  rval = moab.tag_get_handle(
      ...
      MB_TAG_CREAT | MB_TAG_SPARSE | MB_TAG_BYTES | MB_TAG_VARLEN, NULL);
  CHKERRQ_MOAB(rval);
  rval = moab.tag_get_handle(MATERIAL_SET_TAG_NAME, 1, MB_TYPE_INTEGER, bhTag,
                             MB_TAG_SPARSE | MB_TAG_CREAT, &default_val);
  CHKERRQ_MOAB(rval);
  ...
  CHKERRQ_MOAB(rval);
  Tag entity_name_tag;
  rval = moab.tag_get_handle(NAME_TAG_NAME, NAME_TAG_SIZE, MB_TYPE_OPAQUE,
    ...
    if ((cmeshset.cubitBcType & CubitBCType(NODESET | SIDESET | BLOCKSET))
      ...
      if (name.size() > 0) {
        bool success = cubitMeshsets.modify(
            p.first, CubitMeshSets_change_name(moab, name));
        if (!success) {
          SETERRQ(PETSC_COMM_SELF, MOFEM_OPERATION_UNSUCCESSFUL,
...
MeshsetsManager::addEntitiesToMeshset(const CubitBCType cubit_bc_type,
  ...
  EntityHandle meshset = cit->getMeshset();
  rval = moab.add_entities(meshset, ents, nb_ents);
  CHKERRQ_MOAB(rval);
  MoFEMFunctionReturnHot(0);
}
...
MeshsetsManager::getMeshsetsByType(const unsigned int cubit_bc_type,
  ...
  MoFEMFunctionBeginHot;
  CubitMeshsetByType::iterator miit =
      cubitMeshsets.get<CubitMeshSets_mi_tag>().lower_bound(cubit_bc_type);
  CubitMeshsetByType::iterator hi_miit =
  ...
  try {
    ...
    for (_IT_CUBITMESHSETS_BY_SET_TYPE_FOR_LOOP_(m_field, BLOCKSET, it)) {
      ...
      configFileOptionsPtr->add_options()(
          ...
          "Add block set")((prefix + ".id").c_str(),
                           po::value<int>(&block_lists[it->getMeshsetId()].iD)
                               ->default_value(-1),
                           "Id of meshset")(
          (prefix + ".name").c_str(),
      ...
      // Mat elastic
      {
        ...
        configFileOptionsPtr->add_options()(
            ...
            po::value<double>(&block_lists[it->getMeshsetId()]
                                   .matElastic.data.ThermalExpansion)
                ->default_value(-1),
            "Thermal expansion");
      ...
      // Mat interface
      {
        ...
        configFileOptionsPtr->add_options()(
            ...
            po::value<double>(
                &block_lists[it->getMeshsetId()].matInterf.data.ft)
                ->default_value(-1),
            "ft")((prefix + ".interface_Gf").c_str(),
                  po::value<double>(
                      &block_lists[it->getMeshsetId()].matInterf.data.Gf)
                      ->default_value(-1),
                  "Gf");
        // TODO Add users parameters
      }
      ...
      // Displacement bc
      {
        ...
        configFileOptionsPtr->add_options()(
            ...
            po::value<char>(&block_lists[it->getMeshsetId()].dispBc.data.flag6)
                ->default_value(0),
            "flag6")((prefix + ".disp_ux").c_str(),
                     po::value<double>(
                         &block_lists[it->getMeshsetId()].dispBc.data.value1)
                         ->default_value(0),
                     "value1")(
            (prefix + ".disp_uy").c_str(),
            po::value<double>(
                &block_lists[it->getMeshsetId()].dispBc.data.value2)
                ->default_value(0),
            "value2")((prefix + ".disp_uz").c_str(),
                      po::value<double>(
                          &block_lists[it->getMeshsetId()].dispBc.data.value3)
                          ->default_value(0),
                      "value3")(
            (prefix + ".disp_rx").c_str(),
            po::value<double>(
                &block_lists[it->getMeshsetId()].dispBc.data.value4)
                ->default_value(0),
            "value4")((prefix + ".disp_ry").c_str(),
                      po::value<double>(
                          &block_lists[it->getMeshsetId()].dispBc.data.value5)
                          ->default_value(0),
                      "value5")(
            (prefix + ".disp_rz").c_str(),
            po::value<double>(
                &block_lists[it->getMeshsetId()].dispBc.data.value6)
                ->default_value(0),
            "value6");
      }
      // Force BC data
      ...
      // Sideset
      {
        ...
        configFileOptionsPtr->add_options()(
            ...
            po::value<char>(
                ...
                ->default_value(0),
            "flag2")(
            (prefix + ".pressure_magnitude").c_str(),
            po::value<double>(
      ...
      }
      {
        ...
        configFileOptionsPtr->add_options()(
            ...
            po::value<char>(
                ...
                ->default_value(0),
            "flag1")(
            (prefix + ".heatflux_magnitude").c_str(),
            po::value<double>(
    ...
    for (map<int, BlockData>::iterator mit = block_lists.begin();
      ...
      case BLOCKSET: {
        if ((CubitBCType(mit->second.bcType) & cubit_meshset_it->getBcType())
          ...
          ierr =
              addMeshset(mit->second.bcType, mit->second.iD, mit->second.nAme);
          CHKERRG(ierr);
        ...
        // Add attributes
        ierr =
            setAtributes(mit->second.bcType, mit->second.iD, mit->second.aTtr);
        CHKERRG(ierr);
        ...
        if (mit->second.matElastic.data.Young != -1) {
          ierr = setAtributesByDataStructure(mit->second.bcType, mit->second.iD,
      ...
      case NODESET: {
        ...
        if (mit->second.forceBc.data.value1 != 0 ||
          ...
          ierr = setBcData(mit->second.bcType, mit->second.iD,
                           mit->second.forceBc);
          CHKERRG(ierr);
        }
      ...
      case SIDESET: {
        ...
        if (mit->second.cfgBc.data.type != 0) {
          ierr =
              setBcData(mit->second.bcType, mit->second.iD, mit->second.cfgBc);
          CHKERRG(ierr);
        }
      } break;
      default:
