...
MeshsetsManager::query_interface(const MOFEMuuid &uuid,
  ...
  *iface = NULL;
  ...
  PetscErrorCode MeshsetsManager::getTags(int verb) {
    ...
    int default_val = -1;
    rval =
        moab.tag_get_handle(DIRICHLET_SET_TAG_NAME, 1, MB_TYPE_INTEGER, nsTag,
  ...
  PetscErrorCode MeshsetsManager::setAtributes(
    ...
    if (name.size() > 0) {
      bool success = cubitMeshsets.modify(
          cubitMeshsets.project<0>(cit), CubitMeshSets_change_name(moab, name));
      if (!success) {
    ...
    try {
      ...
      for (map<int, BlockData>::iterator mit = block_lists.begin();
        ...
        switch(mit->second.bcType) {
          ...
          break;
          case BLOCKSET: {
            if ((CubitBCType(mit->second.bcType) &
                 cubit_meshset_it->getBcType())
                    .any() &&
