...
MoFEMErrorCode CoordSystemsManager::getTags(int verb) {
  ...
  rval = moab.tag_get_handle("_CoordSysDim", 4, MB_TYPE_INTEGER, th_CoordSysDim,
...
MoFEMErrorCode CoordSystemsManager::initialiseDatabaseFromMesh(int verb) {
  ...
  for (; mit != meshsets.end(); mit++) {
    try {
      const char *cs_name;
      int cs_name_size;
      rval = moab.tag_get_by_ptr(th_CoordSysName, &*mit, 1,
                                 (const void **)&cs_name, &cs_name_size);
      if (rval == MB_SUCCESS && cs_name_size) {
        int dim[4];
        rval = moab.tag_get_data(th_CoordSysDim, &*mit, 1, dim);
        CHKERRQ_MOAB(rval);
  ...
  { // Create cartesian coordinate system if not exist
    ...
    if (csit == coordinateSystems.get<CoordSysName_mi_tag>().end()) {
      ...
      CHKERRQ_MOAB(rval);
      int dim[] = {3, 0, 0, 0};
      rval = moab.tag_set_data(th_CoordSysDim, &meshset, 1, dim);
      CHKERRQ_MOAB(rval);
      std::string sys_name_str = "CARTESIAN3D";
      void const *sys_name[] = {sys_name_str.c_str()};
      int sys_name_size[1];
      sys_name_size[0] = sys_name_str.size();
      rval = moab.tag_set_by_ptr(th_CoordSysName, &meshset, 1, sys_name,
                                 sys_name_size);
      CHKERRQ_MOAB(rval);
      std::pair<CoordSys_multiIndex ::iterator, bool> p =
  ...
  CoordSys_multiIndex::index<CoordSysName_mi_tag>::type::iterator
      undefined_cs_it;
  undefined_cs_it =
      coordinateSystems.get<CoordSysName_mi_tag>().find("UNDEFINED");
  if (undefined_cs_it == coordinateSystems.get<CoordSysName_mi_tag>().end()) {
    SETERRQ(PETSC_COMM_SELF, MOFEM_DATA_INCONSISTENCY,
            "Undefined system not found");
  }
...
MoFEMErrorCode
CoordSystemsManager::addCoordinateSystem(const int cs_dim[],
  ...
  rval = moab.create_meshset(MESHSET_SET | MESHSET_TRACK_OWNER, meshset);
  CHKERRQ_MOAB(rval);
  rval = moab.tag_set_data(th_CoordSysDim, &meshset, 1, cs_dim);
  CHKERRQ_MOAB(rval);
