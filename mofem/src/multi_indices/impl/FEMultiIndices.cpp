...
MoFEMErrorCode DefaultElementAdjacency::defaultTri(
  ...
  switch (field_ptr.getSpace()) {
    ...
    // break;
    case HCURL:
    rval = moab.get_adjacencies(&fe_ent,1,1,false,adjacency,moab::Interface::UNION); CHKERRQ_MOAB(rval);
    // adjacency.insert(edges.begin(),edges.end());
    // for(Range::iterator eeit = edges.begin();eeit!=edges.end();eeit++) {
    //   fe_ptr.getSideNumberPtr(*eeit);
    // }
...
MoFEMErrorCode DefaultElementAdjacency::defaultTet(
  ...
  switch (field_ptr.getSpace()) {
    case H1:
    rval = moab.get_connectivity(&fe_ent,1,adjacency,true); CHKERRQ_MOAB(rval);
    // rval = moab.get_adjacencies(&fe_ent,1,0,false,adjacency,moab::Interface::UNION); CHKERRQ_MOAB(rval);
    // if(adjacency.size()>4){
    //   Range topo_nodes;
    //   rval = moab.get_connectivity(&fe_ent,1,topo_nodes,true); CHKERRQ_MOAB(rval);
    //   Range mid_nodes;
    ...
    rval = moab.get_adjacencies(&fe_ent,1,1,false,adjacency,moab::Interface::UNION); CHKERRQ_MOAB(rval);
    case HDIV:
    rval = moab.get_adjacencies(&fe_ent,1,2,false,adjacency,moab::Interface::UNION); CHKERRQ_MOAB(rval);
    case L2:
    adjacency.insert(fe_ent);
    break;
    case NOFIELD:
...
MoFEMErrorCode DefaultElementAdjacency::defaultPrism(
  ...
  try {
    ...
    for(;nn<3;nn++) {
      ...
      boost::shared_ptr<SideNumber> side_ptr = fe_ptr.getRefElement()->getSideNumberPtr(node);
...
static MoFEMErrorCode get_fe_dof_view(
...
MoFEMErrorCode EntFiniteElement::getElementAdjacency(
...
MoFEMErrorCode NumeredEntFiniteElement::getRowDofsByPetscGlobalDofIdx(DofIdx idx,const FENumeredDofEntity **dof_ptr) const {
  MoFEMFunctionBeginHot;
...
MoFEMErrorCode NumeredEntFiniteElement::getColDofsByPetscGlobalDofIdx(DofIdx idx,const FENumeredDofEntity **dof_ptr) const {
  MoFEMFunctionBeginHot;
  FENumeredDofEntity_multiIndex::index<PetscGlobalIdx_mi_tag>::type::iterator dit;
  dit = rows_dofs->get<PetscGlobalIdx_mi_tag>().find(idx);
  if(dit == rows_dofs->get<PetscGlobalIdx_mi_tag>().end()) {
