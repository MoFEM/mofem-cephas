...
PetscErrorCode MoFEM::Hdiv_FaceBubbleShapeFunctions_MBTET_ON_FACE(
  ...
  for(int ii = 0;ii<gdim;ii++) {
    ...
    FTensor::Tensor1<double*,3> t_diff_psi_m(
      &diff_psi_m[0],&diff_psi_m[p+1],&diff_psi_m[2*p+2],1
    );
    int jj = 0;
    int oo = 0;
    for(;oo<=p-3;oo++) {
      ...
      for(;l<=oo;l++) {
        int m = oo - l;
        if(m>=0) {
          t_psi_f(i) = (beta_0ij*t_psi_l*t_psi_m)*t_cross(i);
          if(diff_phi_f) {
            (*t_diff_phi_f_ptr)(i,j) = (
              (t_psi_l*t_psi_m)*t_diff_beta_0ij(j)+
              (beta_0ij*t_psi_m)*t_diff_psi_l(j)+
