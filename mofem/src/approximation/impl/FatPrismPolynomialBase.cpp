...
FatPrismPolynomialBase::getValue(MatrixDouble &pts,
  ...
  switch (cTx->sPace) {
  case H1:
    ierr = getValueH1TrianglesOnly();
    CHKERRG(ierr);
    ierr = getValueH1ThroughThickness();
    CHKERRG(ierr);
    ierr = getValueH1(pts);
    CHKERRG(ierr);
    break;
...
MoFEMErrorCode FatPrismPolynomialBase::getValueH1ThroughThickness() {
  ...
  for (unsigned int ee = 3; ee <= 5; ee++) {
    ...
    int order =
        cTx->dataTroughThickness.dataOnEntities[MBEDGE][ee].getDataOrder() - 2;
    ...
    for (int gg = 0; gg < nb_gauss_pts_through_thickness; gg++) {
      ...
      ierr = base_polynomials(
          order, s, &diff_s,
          &cTx->dataTroughThickness.dataOnEntities[MBEDGE][ee].getN(base)(gg,
                                                                          0),
          &cTx->dataTroughThickness.dataOnEntities[MBEDGE][ee].getDiffN(base)(
              gg, 0),
          1);
      CHKERRG(ierr);
    }
...
MoFEMErrorCode FatPrismPolynomialBase::getValueH1(MatrixDouble &pts) {
  ...
  try {
    ...
    // higher order edges and zeta
    {
      ...
      rval = cTx->mOab.get_connectivity(ent, conn, num_nodes, true);
      CHKERRQ_MOAB(rval);
      // std::cerr << "\n\n" << std::endl;
      ...
      for (; siit != hi_siit; siit++) {
        ...
        rval =
            cTx->mOab.get_connectivity(quad, conn_quad, num_nodes_quad, true);
        CHKERRQ_MOAB(rval);
        ...
        int order = data.dataOnEntities[MBQUAD][siit->get()->side_number]
                        .getDataOrder();
      ...
      if (quad_order[0] > 0 || quad_order[1] > 0 || quad_order[2] > 0) {
        ...
        ierr = H1_QuadShapeFunctions_MBPRISM(
            quads_nodes, quad_order, vertex_n, diff_vertex_n, quad_n,
            diff_quad_n, nb_gauss_pts, Legendre_polynomials);
        CHKERRG(ierr);
      }
    }
    // prism
