...
MoFEMErrorCode Legendre_polynomials01(int p, double s01, double *L) {
...
MoFEMErrorCode Integrated_Legendre01(int p, double s01, double *L,
                                     double *diffL) {
  MoFEMFunctionBeginHot;
  double l[p + 3];
  CHKERR ::DemkowiczHexAndQuad::Legendre_polynomials01(p, s01, l);
  if (p >= 2) {
    for (int i = 2; i != p + 1; i++) {
...
MoFEMErrorCode MoFEM::DemkowiczHexAndQuad::H1_FaceShapeFunctions_ONHEX(
  ...
  for (int face = 0; face != 6; face++) {
    ...
    for (int qq = 0; qq != nb_integration_pts; qq++) {
      ...
      CHKERR Lobatto_polynomials(p[0] + 1, ksi01, diff_ksi01, L01, diffL01, 3);
      double L12[p[1] + 2];
      double diffL12[3 * (p[1] + 2)];
      CHKERR Lobatto_polynomials(p[1] + 1, ksi12, diff_ksi12, L12, diffL12, 3);

      int qd_shift = (p[face] - 1) * (p[face] - 1) * qq;
