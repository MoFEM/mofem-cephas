...
struct HcurlEdgeBase {
  ...
  MoFEMErrorCode
  operator()(int shift, int p, int nb_integration_pts, int n0_idx, int n1_idx,
             double n[], FTensor::Tensor1<double, 3> t_grad_n[],
             FTensor::Tensor1<FTensor::PackPtr<double *, 3>, 3> &t_phi) {
...
MoFEMErrorCode MoFEM::Hcurl_Demkowicz_EdgeBaseFunctions_MBTET(
  ...
  for (int ee = 0; ee != 6; ++ee) {
    ...
    FTensor::Tensor1<FTensor::PackPtr<double *, 3>, 3> t_phi(
        &phi[ee][HVEC0], &phi[ee][HVEC1], &phi[ee][HVEC2]);

    FTensor::Tensor2<FTensor::PackPtr<double *, 9>, 3, 3> t_diff_phi(
    ...
    CHKERR h_curl_base_on_edge.calculate<3>(p[ee], nb_integration_pts, n0_idx,
                                            n1_idx, n, t_grad_n, t_phi,
                                            &t_diff_phi);
  }
...
MoFEMErrorCode MoFEM::Hcurl_Demkowicz_EdgeBaseFunctions_MBTRI(
  ...
  for (int ee = 0; ee != 3; ++ee) {
    ...
    CHKERR h_curl_base_on_edge.calculate<2>(p[ee], nb_integration_pts, n0_idx,
                                            n1_idx, n, t_grad_n, t_phi,
                                            &t_diff_phi);
  }
...
MoFEMErrorCode MoFEM::Hcurl_Demkowicz_EdgeBaseFunctions_MBEDGE(
  ...
  CHKERR h_curl_base_on_edge.calculate<1>(p, nb_integration_pts, n0_idx, n1_idx,
                                          n, t_grad_n, t_phi, nullptr);
    ...
    for (int oo = 3; oo <= p; ++oo) {
      ...
      for (int oo_ij = 2; oo_ij != oo; ++oo_ij) {
        ...
        FTensor::Tensor1<double, 3> t_diff_fi_k_f0(diff_fi_k(0, 0 + k - 1),
                                                   diff_fi_k(0, k + k - 1),
                                                   diff_fi_k(0, 2 * k + k - 1));
        FTensor::Tensor1<double, 3> t_diff_fi_k_f1(diff_fi_k(1, 0 + k - 1),
                                                   diff_fi_k(1, k + k - 1),
                                                   diff_fi_k(1, 2 * k + k - 1));
        FTensor::Tensor1<double, 3> t_diff_fi_k_f2(diff_fi_k(2, 0 + k - 1),
                                                   diff_fi_k(2, k + k - 1),
                                                   diff_fi_k(2, 2 * k + k - 1));
