...
PetscErrorCode Hdiv_EdgeFaceShapeFunctions_MBTET(
  ...
  for(;ff<4;ff++) {
    if(diffPHI_f_e!=NULL) {
      ierr = Hdiv_EdgeFaceShapeFunctions_MBTET_ON_FACE(&faces_nodes[3*ff],p[ff],N,diffN,PHI_f_e[ff],diffPHI_f_e[ff],GDIM,4); CHKERRQ(ierr);
    } else {
      ierr = Hdiv_EdgeFaceShapeFunctions_MBTET_ON_FACE(&faces_nodes[3*ff],p[ff],N,diffN,PHI_f_e[ff],NULL,GDIM,4); CHKERRQ(ierr);
    }
  }
...
PetscErrorCode Hdiv_FaceBubbleShapeFunctions_MBTET(
  ...
  for(;ff<4;ff++) {
    ...
    ierr = Hdiv_FaceBubbleShapeFunctions_MBTET_ON_FACE(&faces_nodes[3*ff],p[ff],N,diffN,PHI_f[ff],diff,GDIM,4); CHKERRQ(ierr);
  }
  PetscFunctionReturn(0);
}
PetscErrorCode Hdiv_EdgeBasedVolumeShapeFunctions_MBTET(
  ...
  for(;ii<GDIM;ii++) {
	...
	if(diffPHI_v_e!=NULL) {
      ...
      if(l!=NBVOLUMETET_EDGE_HDIV(p)) SETERRQ2(PETSC_COMM_SELF,MOFEM_DATA_INCONSISTENCT,"wrong order %d != %d",l,NBVOLUMETET_FACE_HDIV(p));
    }
  }
  PetscFunctionReturn(0);
}
PetscErrorCode Hdiv_FaceBasedVolumeShapeFunctions_MBTET(
  ...
  for(;ii<GDIM;ii++) {
	...
	for(;l<=oo;l++) {
      ...
      if(jj!=NBVOLUMETET_FACE_HDIV(p)) SETERRQ2(PETSC_COMM_SELF,MOFEM_DATA_INCONSISTENCT,"wrong order %d != %d",jj,NBVOLUMETET_FACE_HDIV(p));
    }
  }
  PetscFunctionReturn(0);
}
PetscErrorCode Hdiv_VolumeBubbleShapeFunctions_MBTET(
  ...
  for(;ii<GDIM;ii++) {
    ...
    if(3*jj!=NBVOLUMETET_VOLUME_HDIV(p)) SETERRQ2(PETSC_COMM_SELF,MOFEM_DATA_INCONSISTENCT,"wrong order %d != %d",jj,NBVOLUMETET_VOLUME_HDIV(p));
  }
  PetscFunctionReturn(0);
}
...
PetscErrorCode Hdiv_EdgeFaceShapeFunctions_MBTET_ON_FACE(
  ...
  for(;ii<GDIM;ii++) {
	...
	if(diffPHI_f_e!=NULL) {
      ...
      if(l!=NBFACETRI_EDGE_HDIV(p)) SETERRQ2(PETSC_COMM_SELF,MOFEM_DATA_INCONSISTENCT,"wrong order %d != %d",l,NBFACETRI_EDGE_HDIV(p));
    }
  }
  PetscFunctionReturn(0);
}
PetscErrorCode Hdiv_FaceBubbleShapeFunctions_MBTET_ON_FACE(
  ...
  for(;ii<GDIM;ii++) {
    ...
    if(jj!=NBFACETRI_FACE_HDIV(p)) SETERRQ2(PETSC_COMM_SELF,MOFEM_DATA_INCONSISTENCT,"wrong order %d != %d",jj,NBFACETRI_FACE_HDIV(p));
  }
  PetscFunctionReturn(0);
}
