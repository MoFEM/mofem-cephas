...
VolumeElementForcesAndSourcesCore::getSpaceBaseAndOrderOnElement() {
  ...
  try {
    ...
    if ((dataH1.spacesOnEntities[MBTET]).test(H1)) {
      ierr = getTetDataOrder(dataH1, H1);
      CHKERRG(ierr);
    }
    // Hcurl
    ...
    if ((dataH1.spacesOnEntities[MBTRI]).test(HCURL)) {
      ...
      CHKERRG(ierr);
      ierr = getFaceTriNodes(dataHcurl);
      CHKERRG(ierr);
    ...
    if ((dataH1.spacesOnEntities[MBTET]).test(HCURL)) {
      ierr = getTetDataOrder(dataHcurl, HCURL);
      CHKERRG(ierr);
    ...
    if ((dataH1.spacesOnEntities[MBTET]).test(HDIV)) {
      ierr = getTetDataOrder(dataHdiv, HDIV);
      CHKERRG(ierr);
      dataHdiv.spacesOnEntities[MBTET].set(HDIV);
    }
    // L2
    if ((dataH1.spacesOnEntities[MBTET]).test(L2)) {
      ierr = getTetDataOrder(dataL2, L2);
      CHKERRG(ierr);
...
MoFEMErrorCode VolumeElementForcesAndSourcesCore::operator()() {
  ...
  try {
    ...
    ierr = calculateVolumeAndJacobian();
    CHKERRG(ierr);
    ...
    ierr = calculateHoJacobian();
    CHKERRG(ierr);
    ierr = transformHoBaseFunctions();
    CHKERRG(ierr);
        ...
        for (int ss = 0; ss != 2; ss++) {
          ...
          if (oit->getOpType() & types[ss] ||
            ...
            if (last_eval_field_name[ss] != field_name) {
              ...
              case L2:
                if (!ss) {
                  ierr = getTetsRowIndices(*op_data[ss], field_name);
                  CHKERRG(ierr);
                } else {
                  ierr = getTetsColIndices(*op_data[ss], field_name);
                  CHKERRG(ierr);
                }
                ierr = getTetDataOrderSpaceAndBase(*op_data[ss], field_name);
                CHKERRG(ierr);
                ierr = getTetsFieldData(*op_data[ss], field_name);
                CHKERRG(ierr);
                break;
              case NOFIELD:
                if (!getNinTheLoop()) {
