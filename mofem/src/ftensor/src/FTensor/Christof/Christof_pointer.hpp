...
namespace FTensor
{
  ...
  public:
    ...
    template <class... U>
    explicit Christof(const int i, U *... d) : data{d...}, inc(i) {
      static_assert(sizeof...(d) == sizeof(data) / sizeof(T),
                    "Incorrect number of Arguments. Constructor should "
                    "initialize the entire Tensor");
    }
  ...
  public Christof<T *, Tensor_Dim0, Tensor_Dim12>
  {

  public:
    template <class... U>
    Christof(U *... d) : Christof<T *, Tensor_Dim0, Tensor_Dim12>(d...) {}

    Christof() : Christof<T *, Tensor_Dim0, Tensor_Dim12>() {}

    /* The ++ operator increments the pointer, not the number that the
       pointer points to.  This allows iterating over a grid. */

    const Christof<PackPtr<T *, I>, Tensor_Dim0, Tensor_Dim12> &
    operator++() const {
      for (int i = 0; i < Tensor_Dim0; ++i)
        for (int j = 0; j < (Tensor_Dim12 * (Tensor_Dim12 + 1)) / 2; ++j)
          Christof<T *, Tensor_Dim0, Tensor_Dim12>::data[i][j] += I;
      return *this;
    }
  };
}
