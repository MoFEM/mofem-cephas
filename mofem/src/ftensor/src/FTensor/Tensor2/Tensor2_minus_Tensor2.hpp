...
namespace FTensor
{
  ...
  inline const Tensor2_Expr<
    const Tensor2_minus_Tensor2_01<A, B, T, U, Dim0, Dim1, i, j>,
    typename promote<T, U>::V, Dim0, Dim1, i, j>
  operator-(const Tensor2_Expr<A, T, Dim0, Dim1, i, j> &a,
            const Tensor2_Expr<B, U, Dim0, Dim1, i, j> &b)
  {
    typedef const Tensor2_minus_Tensor2_01<A, B, T, U, Dim0, Dim1, i, j>
      TensorExpr;
  ...
  template <class A, class B, class T, class U, int Dim0, int Dim1, char i,
            char j>
  inline const Tensor2_Expr<
    const Tensor2_minus_Tensor2_10<A, B, T, U, Dim0, Dim1, i, j>,
    typename promote<T, U>::V, Dim0, Dim1, i, j>
  operator-(const Tensor2_Expr<A, T, Dim0, Dim1, i, j> &a,
            const Tensor2_Expr<B, U, Dim0, Dim1, j, i> &b)
  {
    typedef const Tensor2_minus_Tensor2_10<A, B, T, U, Dim0, Dim1, i, j>
      TensorExpr;
    return Tensor2_Expr<TensorExpr, typename promote<T, U>::V, Dim0, Dim1, i,
                        j>(TensorExpr(a, b));
  }
