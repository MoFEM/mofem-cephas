...
namespace FTensor
{
  ...
  inline const Tensor2_Expr<
    const Tensor2_and_Tensor1_0<A, B, T, U, Dim0, Dim1, i, j>,
    typename promote<T, U>::V, Dim0, Dim1, i, j>
  operator&(const Tensor2_Expr<A, T, Dim0, Dim1, i, j> &a,
            const Tensor1_Expr<B, U, Dim0, i> &b)
  {
    typedef const Tensor2_and_Tensor1_0<A, B, T, U, Dim0, Dim1, i, j>
      TensorExpr;
  ...
  template <class A, class B, class T, class U, int Dim0, int Dim1, char i,
            char j>
  inline const Tensor2_Expr<
    const Tensor2_and_Tensor1_1<A, B, T, U, Dim0, Dim1, i, j>,
    typename promote<T, U>::V, Dim0, Dim1, i, j>
  operator&(const Tensor2_Expr<A, T, Dim0, Dim1, i, j> &a,
            const Tensor1_Expr<B, U, Dim1, j> &b)
  {
    typedef const Tensor2_and_Tensor1_1<A, B, T, U, Dim0, Dim1, i, j>
      TensorExpr;
    return Tensor2_Expr<TensorExpr, typename promote<T, U>::V, Dim0, Dim1, i,
                        j>(TensorExpr(a, b));
  }
