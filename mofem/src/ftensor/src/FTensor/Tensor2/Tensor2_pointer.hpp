...
template <class T, int Tensor_Dim0, int Tensor_Dim1, Layout layout, int I>
class Tensor2<PackPtr<T *, I>, Tensor_Dim0, Tensor_Dim1, layout> {
  mutable T *restrict
      data[(layout == column_major) ? Tensor_Dim0 : Tensor_Dim1]
...
public:
  ...
  Tensor2_Expr<Tensor2<PackPtr<T *, I>, Tensor_Dim0, Tensor_Dim1, layout>, T,
               Dim0, Dim1, i, j>
  operator()(const Index<i, Dim0>, const Index<j, Dim1> index2) {
    return Tensor2_Expr<
        Tensor2<PackPtr<T *, I>, Tensor_Dim0, Tensor_Dim1, layout>, T, Dim0,
        Dim1, i, j>(*this);
  }

  template <char i, char j, int Dim0, int Dim1>
  Tensor2_Expr<const Tensor2<PackPtr<T *, I>, Tensor_Dim0, Tensor_Dim1, layout>,
               T, Dim0, Dim1, i, j>
  operator()(const Index<i, Dim0>, const Index<j, Dim1> index2) const {
    return Tensor2_Expr<
        const Tensor2<PackPtr<T *, I>, Tensor_Dim0, Tensor_Dim1, layout>, T,
        Dim0, Dim1, i, j>(*this);
  }
  ...
  Tensor1_Expr<
      Tensor2_number_rhs_0<
          Tensor2<PackPtr<T *, I>, Tensor_Dim0, Tensor_Dim1, layout>, T, N>,
      T, Dim, i>
  ...
  template <char i, int Dim, int N>
  Tensor1_Expr<
      const Tensor2_number_1<
          const Tensor2<PackPtr<T *, I>, Tensor_Dim0, Tensor_Dim1, layout>, T,
          N>,
      T, Dim, i>
  operator()(const Index<i, Dim>, const Number<N>) const {
    typedef const Tensor2_number_1<
        const Tensor2<PackPtr<T *, I>, Tensor_Dim0, Tensor_Dim1, layout>, T, N>
        TensorExpr;
  ...
  const Tensor2<PackPtr<T *, I>, Tensor_Dim0, Tensor_Dim1, layout> &
  operator++() const {
    for (int i = 0; i < Tensor_Dim0; ++i)
      for (int j = 0; j < Tensor_Dim1; ++j)
        ((layout == column_major) ? data[i][j] += I : data[j][i] += I);
    return *this;
  }
