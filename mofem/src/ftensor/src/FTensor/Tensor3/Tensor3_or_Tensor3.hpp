...
namespace FTensor
{
  ...
  operator||(const Tensor3_Expr<A, T, Dim0, Dim12, Dim12, i, j, k> &a,
             const Tensor3_Expr<B, U, Dim0, Dim12, Dim12, i, k, j> &b)
  {
    typedef const Tensor3_or_Tensor3_12<A, B, T, U, Dim0, Dim12, i, j, k>
      TensorExpr;
    return Dg_Expr<TensorExpr, typename promote<T, U>::V, Dim0, Dim12, j, k, i>(
      TensorExpr(a, b));
  }
  ...
  operator||(const Tensor3_Expr<A, T, Dim02, Dim1, Dim02, i, j, k> &a,
             const Tensor3_Expr<B, U, Dim02, Dim1, Dim02, k, j, i> &b)
  {
    typedef const Tensor3_or_Tensor3_02<A, B, T, U, Dim02, Dim1, i, j, k>
      TensorExpr;
    return Dg_Expr<TensorExpr, typename promote<T, U>::V, Dim02, Dim1, i, k, j>(
      TensorExpr(a, b));
  }
}
