...
namespace FTensor
{
  ...
  operator%(const Tensor2_symmetric_Expr<A, T, Dim, i, j> &a,
            const Tensor2_symmetric_Expr<B, U, Dim, i, j> &b)
  {
    typedef const Tensor2_symmetric_mod_Tensor2_symmetric_01<A, B, T, U, Dim,
                                                             i, j>
      TensorExpr;
  ...
  template <class A, class B, class T, class U, int Dim, char i, char j>
  inline const Tensor2_symmetric_Expr<
    const Tensor2_symmetric_mod_Tensor2_symmetric_10<A, B, T, U, Dim, i, j>,
    typename promote<T, U>::V, Dim, i, j>
  operator%(const Tensor2_symmetric_Expr<A, T, Dim, i, j> &a,
            const Tensor2_symmetric_Expr<B, U, Dim, j, i> &b)
  {
    typedef const Tensor2_symmetric_mod_Tensor2_symmetric_10<A, B, T, U, Dim,
                                                             i, j>
      TensorExpr;
    return Tensor2_symmetric_Expr<TensorExpr, typename promote<T, U>::V, Dim,
                                  i, j>(TensorExpr(a, b));
