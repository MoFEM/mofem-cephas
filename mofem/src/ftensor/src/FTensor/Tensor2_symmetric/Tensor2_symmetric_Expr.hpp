...
namespace FTensor
{
  ...
  public:
    ...
    const Tensor2_symmetric_Expr<Tensor2_symmetric<A, Tensor_Dim>, T, Dim, i, j>
        &operator<<=(const Tensor2_symmetric_Expr<
                     Tensor2_symmetric<A, Tensor_Dim>, T, Dim, i, j> &result);

    template <class B, class U>
    inline const Tensor2_symmetric_Expr<Tensor2_symmetric<A, Tensor_Dim>, T,
                                        Dim, i, j> &
    operator<<=(const Tensor2_symmetric_Expr<B, U, Dim, j, i> &result);
  ...
  public:
    ...
    template<class B, class U>
    const Tensor2_symmetric_Expr<Dg_number_rhs_2<A,T,N>,
                                 T,Dim,i,j> &
    operator=(const Tensor2_symmetric_Expr<B,U,Dim,i,j> &result);

    const Tensor2_symmetric_Expr<Dg_number_rhs_2<A,T,N>,
                                 T,Dim,i,j> &
    operator=(const Tensor2_symmetric_Expr<Dg_number_rhs_2<A,T,N>,
              T,Dim,i,j> &result);
  };
  ...
  public:
    ...
    template<class B, class U>
    const Tensor2_symmetric_Expr<Ddg_number_rhs_01<A,T,N0,N1>,
                                 T,Dim,i,j> &
    operator=(const Tensor2_symmetric_Expr<B,U,Dim,i,j> &result);
