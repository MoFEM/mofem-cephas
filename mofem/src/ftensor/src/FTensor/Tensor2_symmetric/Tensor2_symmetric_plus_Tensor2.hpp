...
namespace FTensor
{
  ...
  inline const Tensor2_Expr<
    const Tensor2_symmetric_plus_Tensor2_01<A, B, T, U, Dim, i, j>,
    typename promote<T, U>::V, Dim, Dim, i, j>
  operator+(const Tensor2_symmetric_Expr<A, T, Dim, i, j> &a,
            const Tensor2_Expr<B, U, Dim, Dim, i, j> &b)
  {
    typedef const Tensor2_symmetric_plus_Tensor2_01<A, B, T, U, Dim, i, j>
      TensorExpr;
  ...
  template <class A, class B, class T, class U, int Dim, char i, char j>
  inline const Tensor2_Expr<
    const Tensor2_symmetric_plus_Tensor2_10<A, B, T, U, Dim, i, j>,
    typename promote<T, U>::V, Dim, Dim, i, j>
  operator+(const Tensor2_symmetric_Expr<A, T, Dim, i, j> &a,
            const Tensor2_Expr<B, U, Dim, Dim, j, i> &b)
  {
    typedef const Tensor2_symmetric_plus_Tensor2_10<A, B, T, U, Dim, i, j>
      TensorExpr;
    return Tensor2_Expr<TensorExpr, typename promote<T, U>::V, Dim, Dim, i, j>(
      TensorExpr(a, b));
  }
