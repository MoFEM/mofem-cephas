...
namespace FTensor
{
  ...
  public:
    ...
    template <char i, char j, int Dim0, int Dim1>
    Tensor2_Expr<Tensor2_symmetric<TPtr, Tensor_Dim>, T, Dim0, Dim1, i, j>
    operator()(const Index<i, Dim0> index1, const Index<j, Dim1> index2)
    {
      return Tensor2_Expr<Tensor2_symmetric<TPtr, Tensor_Dim>, T, Dim0, Dim1, i,
                          j>(*this);
    }

    template <char i, char j, int Dim0, int Dim1>
    Tensor2_Expr<const Tensor2_symmetric<TPtr, Tensor_Dim>, T, Dim0, Dim1, i, j>
    operator()(const Index<i, Dim0> index1, const Index<j, Dim1> index2) const
    {
      return Tensor2_Expr<const Tensor2_symmetric<TPtr, Tensor_Dim>, T, Dim0,
                          Dim1, i, j>(*this);
    }
    ...
    template <char i, char j, int Dim>
    Tensor2_symmetric_Expr<Tensor2_symmetric<TPtr, Tensor_Dim>, T, Dim, i, j>
    operator()(const Index<i, Dim> index1, const Index<j, Dim> index2)
    {
      return Tensor2_symmetric_Expr<Tensor2_symmetric<TPtr, Tensor_Dim>, T, Dim,
                                    i, j>(*this);
    }

    template <char i, char j, int Dim>
    Tensor2_symmetric_Expr<const Tensor2_symmetric<TPtr, Tensor_Dim>, T, Dim, i,
                           j>
    operator()(const Index<i, Dim> index1, const Index<j, Dim> index2) const
    {
      return Tensor2_symmetric_Expr<const Tensor2_symmetric<TPtr, Tensor_Dim>,
                                    T, Dim, i, j>(*this);
    }
    ...
    template <char i, int N, int Dim>
    Tensor1_Expr<Tensor2_number_rhs_1<Tensor2_symmetric<TPtr, Tensor_Dim>, T, N>,
                 T, Dim, i>
    operator()(const Index<i, Dim> index1, const Number<N> &n1)
    {
      using TensorExpr
        = Tensor2_number_rhs_1<Tensor2_symmetric<TPtr, Tensor_Dim>, T, N>;
      return Tensor1_Expr<TensorExpr, T, Dim, i>(*this);
    }

    template <char i, int N, int Dim>
    Tensor1_Expr<
      const Tensor2_number_1<const Tensor2_symmetric<TPtr, Tensor_Dim>, T, N>,
      T, Dim, i>
    operator()(const Index<i, Dim> index1, const Number<N> &n1) const
    {
      using TensorExpr
        = Tensor2_number_1<const Tensor2_symmetric<TPtr, Tensor_Dim>, T, N>;
      return Tensor1_Expr<TensorExpr, T, Dim, i>(TensorExpr(*this));
    }

    template <char i, int N, int Dim>
    Tensor1_Expr<Tensor2_number_rhs_0<Tensor2_symmetric<TPtr, Tensor_Dim>, T, N>,
                 T, Dim, i>
    operator()(const Number<N> &n1, const Index<i, Dim> index1)
    {
      using TensorExpr
        = Tensor2_number_rhs_0<Tensor2_symmetric<TPtr, Tensor_Dim>, T, N>;
      return Tensor1_Expr<TensorExpr, T, Dim, i>(*this);
    }

    template <char i, int N, int Dim>
    Tensor1_Expr<
      const Tensor2_number_0<const Tensor2_symmetric<TPtr, Tensor_Dim>, T, N>,
      T, Dim, i>
    operator()(const Number<N> &n1, const Index<i, Dim> index1) const
    {
      using TensorExpr
        = Tensor2_number_0<const Tensor2_symmetric<TPtr, Tensor_Dim>, T, N>;
      return Tensor1_Expr<TensorExpr, T, Dim, i>(TensorExpr(*this));
    }
    ...
    template <char i, int Dim>
    Tensor1_Expr<
      const Tensor2_numeral_1<const Tensor2_symmetric<TPtr, Tensor_Dim>, T>, T,
      Dim, i>
    operator()(const Index<i, Dim> index1, const int N) const
    {
      using TensorExpr
        = Tensor2_numeral_1<const Tensor2_symmetric<TPtr, Tensor_Dim>, T>;
      return Tensor1_Expr<TensorExpr, T, Dim, i>(TensorExpr(*this, N));
    }

    template <char i, int Dim>
    Tensor1_Expr<
      const Tensor2_numeral_0<const Tensor2_symmetric<TPtr, Tensor_Dim>, T>, T,
      Dim, i>
    operator()(const int N, const Index<i, Dim> index1) const
    {
      using TensorExpr
        = Tensor2_numeral_0<const Tensor2_symmetric<TPtr, Tensor_Dim>, T>;
      return Tensor1_Expr<TensorExpr, T, Dim, i>(TensorExpr(*this, N));
    }
