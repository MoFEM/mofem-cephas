...
namespace FTensor
{
  ...
  class Tensor2_symmetric<PackPtr<T *, I>, Tensor_Dim>
      : public Tensor2_symmetric<T *, Tensor_Dim> {

  public:
    template <class... U>
    Tensor2_symmetric(U *... d) : Tensor2_symmetric<T *, Tensor_Dim>(d...) {}

    Tensor2_symmetric(): Tensor2_symmetric<T *, Tensor_Dim>() {}

    /* The ++ operator increments the pointer, not the number that the
       pointer points to.  This allows iterating over a grid. */

    const Tensor2_symmetric<PackPtr<double, I>, Tensor_Dim> &
    operator++() const {
      for(int i = 0; i < (Tensor_Dim * (Tensor_Dim + 1)) / 2; ++i)
        Tensor2_symmetric<T *, Tensor_Dim>::data[i] += I;
      return *this;
    }
