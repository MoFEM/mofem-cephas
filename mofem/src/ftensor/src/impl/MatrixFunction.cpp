...
template <typename T>
FTensor::Tensor2_symmetric<T, 3> getMatImpl(Val<T> &t_val, Vec<T> &t_vec, Fun<T> f,
                                        const int nb) {
  switch (nb) {
  case 1:
    return EigenProjection<T, T, 1>(t_val, t_vec).getMat(f);
  case 2:
    return EigenProjection<T, T, 2>(t_val, t_vec).getMat(f);
  case 3:
    break;
  default:
...
template <typename T>
FTensor::Ddg<T, 3, 3> getDiffMatImpl(Val<T> &t_val, Vec<T> &t_vec, Fun<T> f,
                                 Fun<T> d_f, const int nb) {
  switch (nb) {
  case 1:
    return EigenProjection<T, T, 1>(t_val, t_vec).getDiffMat(f, d_f);
  case 2:
    return EigenProjection<T, T, 2>(t_val, t_vec).getDiffMat(f, d_f);
  case 3:
    break;
  default:
    THROW_MESSAGE("third parameter should be 1,2 or 3");
...
FTensor::Ddg<T, 3, 3> getDiffDiffMatImpl(Val<T> &t_val, Vec<T> &t_vec, Fun<T> f,
  ...
  switch (nb) {
  case 1:
    return EigenProjection<T, T, 1>(t_val, t_vec)
        .getDiffDiffMat(f, d_f, dd_f, t_S);
  case 2:
    return EigenProjection<T, T, 2>(t_val, t_vec)
        .getDiffDiffMat(f, d_f, dd_f, t_S);
  case 3:
    break;
  default:
