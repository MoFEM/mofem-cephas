...
BitFEId moabField_Core::get_BitFEId() {
  assert((unsigned int)*MoFEMFE_shift<BitFEId().set().to_ulong());
  return BitFEId(1<<(((*MoFEMFE_shift)++)-1)); 
}
...
PetscErrorCode moabField_Core::add_BitFieldId(const string& name,const BitFieldId id,const FieldSpace space,const ApproximationRank rank,int verb) {
  ...
  *build_MoFEM = 0;
  ...
  if(space == NoField) {
    pair<RefMoFEMEntity_multiIndex::iterator,bool> p_ref_ent = ref_entities.insert(RefMoFEMEntity(moab,meshset));
...
PetscErrorCode moabField_Core::map_from_mesh(int verb) {
  ...
  for(;mit!=meshsets.end();mit++) {
    ...
    rval = moab.tag_get_data(th_FieldId,&*mit,1,&BitFieldId); CHKERR_PETSC(rval);
    if(BitFieldId!=0) {
      pair<MoFEMField_multiIndex::iterator,bool> p;
      try {
    ...
    BitProblemId BitProblemId;
    rval = moab.tag_get_data(th_ProblemId,&*mit,1,&BitProblemId); CHKERR_PETSC(rval);
    if(BitProblemId!=0) {
  ...
  *build_MoFEM = 0;
  ...
  for(unsigned int ee = 0;ee<ents.size();ee++,eit++) {
    ...
    if(miit3!=ents_id_view.end()) {
      ...
      MoFEMEntity_multiIndex::iterator miit4 
	= ents_moabfield.get<Unique_mi_tag>().find((*miit3)->get_unique_id());
...
PetscErrorCode moabField_Core::dofs_L2H1HcurlHdiv(const BitFieldId id,int verb) {
  ...
  for(;eit!=ents_of_id_meshset.end();eit++) {
    ...
    try {
      MoFEMEntity moabent(moab,&*miit,&*miit_ref_ent);
      // insert moabent into mofem databse
      if(moabent.get_order_nb_dofs(moabent.get_max_order())==0) continue; 
      e_miit = ents_moabfield.insert(moabent);
    } catch (const char* msg) {
      SETERRQ(PETSC_COMM_SELF,1,msg);
    ...
    int DD = 0;
    int oo = 0;
    for(;oo<=e_miit.first->get_max_order();oo++) {
      for(int dd = 0;dd<e_miit.first->get_order_nb_dofs_diff(oo);dd++) {
	for(int rr = 0;rr<e_miit.first->get_max_rank();rr++,DD++) {
	  d_miit.first = dofs_moabfield.project<0>(
	    dofs_moabfield.get<Unique_mi_tag>().find(DofMoFEMEntity::get_unique_id_calculate(DD,&*(e_miit.first)))
	  );
	  if(d_miit.first==dofs_moabfield.end()) {
	    DofMoFEMEntity mdof(&*(e_miit.first),oo,rr,DD);
	    d_miit = dofs_moabfield.insert(mdof);
	    bool success = dofs_moabfield.modify(d_miit.first,DofMoFEMEntity_active_change(true));
	    if(!success) SETERRQ(PETSC_COMM_SELF,1,"modification unsucceeded");
	  } 
	  //check ent
	  assert(d_miit.first->get_ent()==e_miit.first->get_ent());
	  assert(d_miit.first->get_ent_type()==e_miit.first->get_ent_type());
	  assert(d_miit.first->get_id()==e_miit.first->get_id());
	  //check dof
	  assert(d_miit.first->get_dof_order()==oo);
	  assert(d_miit.first->get_max_order()==e_miit.first->get_max_order());
	}
      }
    }
    assert(DD/e_miit.first->get_max_rank() == e_miit.first->get_order_nb_dofs(e_miit.first->get_max_order()));
    ...
    dof_set_by_identity::iterator miit2 = set_identity.lower_bound(e_miit.first->get_unique_id(),comp_DofMoFEMEntity_ent_uid());
    dof_set_by_identity::iterator hi_miit2 = set_identity.upper_bound(e_miit.first->get_unique_id(),comp_DofMoFEMEntity_ent_uid());
    for(;miit2!=hi_miit2;miit2++) {
      if(miit2->get_dof_order()<=e_miit.first->get_max_order()) continue;
      bool success = dofs_moabfield.modify(miit2,DofMoFEMEntity_active_change(false));
      if(!success) SETERRQ(PETSC_COMM_SELF,1,"modification unsucceeded");
    }
  }
  PetscFunctionReturn(0);
...
PetscErrorCode moabField_Core::add_MoFEMFE(const string &MoFEMFE_name) {
  ...
  *build_MoFEM &= 1<<0;
  ...
  finite_elements_by_name &MoFEMFE_name_set = finite_elements.get<MoFEMFE_name_mi_tag>();
  finite_elements_by_name::iterator it_MoFEMFE = MoFEMFE_name_set.find(MoFEMFE_name);
  if(it_MoFEMFE!=MoFEMFE_name_set.end()) SETERRQ(PETSC_COMM_SELF,1,"this MoFEMFE is there");
  EntityHandle meshset;
  rval = moab.create_meshset(MESHSET_SET|MESHSET_TRACK_OWNER,meshset); CHKERR_PETSC(rval);
  //id
  ...
  //id name
  void const* tag_data[] = { MoFEMFE_name.c_str() };
  int tag_sizes[] = { MoFEMFE_name.size() };
  ...
  //add MoFEMFE
  pair<MoFEMFE_multiIndex::iterator,bool> p = finite_elements.insert(MoFEMFE(moab,meshset));
  if(!p.second) SETERRQ(PETSC_COMM_SELF,1,"MoFEMFE not inserted");
  if(verbose>0) {
    ostringstream ss;
...
PetscErrorCode moabField_Core::modify_MoFEMFE_data_add_bit(const string &MoFEMFE_name,const string &name_data) {
  ...
  *build_MoFEM &= 1<<0;
  ...
  finite_elements_by_name::iterator it_MoFEMFE = MoFEMFE_name_set.find(MoFEMFE_name);
  if(it_MoFEMFE==MoFEMFE_name_set.end()) SETERRQ(PETSC_COMM_SELF,1,"this MoFEMFE is there");
  bool success = MoFEMFE_name_set.modify(it_MoFEMFE,EntMoFEMFE_change_bit_add(get_BitFieldId(name_data)));
  if(!success) SETERRQ(PETSC_COMM_SELF,1,"modification unsucceeded");
  PetscFunctionReturn(0);
}
PetscErrorCode moabField_Core::modify_MoFEMFE_row_add_bit(const string &MoFEMFE_name,const string &name_row) {
  ...
  *build_MoFEM &= 1<<0;
  ...
  finite_elements_by_name::iterator it_MoFEMFE = MoFEMFE_name_set.find(MoFEMFE_name);
  if(it_MoFEMFE==MoFEMFE_name_set.end()) SETERRQ(PETSC_COMM_SELF,1,"this MoFEMFE is there");
  bool success = MoFEMFE_name_set.modify(it_MoFEMFE,MoFEMFE_row_change_bit_add(get_BitFieldId(name_row)));
  if(!success) SETERRQ(PETSC_COMM_SELF,1,"modification unsucceeded");
  PetscFunctionReturn(0);
}
PetscErrorCode moabField_Core::modify_MoFEMFE_col_add_bit(const string &MoFEMFE_name,const string &name_col) {
  ...
  *build_MoFEM &= 1<<0;
  ...
  finite_elements_by_name::iterator it_MoFEMFE = MoFEMFE_name_set.find(MoFEMFE_name);
  if(it_MoFEMFE==MoFEMFE_name_set.end()) SETERRQ(PETSC_COMM_SELF,1,"this MoFEMFE is there");
  bool success = MoFEMFE_name_set.modify(it_MoFEMFE,MoFEMFE_col_change_bit_add(get_BitFieldId(name_col)));
  if(!success) SETERRQ(PETSC_COMM_SELF,1,"modification unsucceeded");
  PetscFunctionReturn(0);
}
...
PetscErrorCode moabField_Core::problem_partition(const BitProblemId id) {
  PetscFunctionBegin;
  MoFEMProblem_multiIndex::index<BitProblemId_mi_tag>::type::iterator problem_it =  problems.get<BitProblemId_mi_tag>().find(id);
  if(problem_it==problems.get<BitProblemId_mi_tag>().end()) SETERRQ(PETSC_COMM_SELF,1,"this problem is not found");
...
PetscErrorCode moabField_Core::modify_problem_MoFEMFE_add_bit(const string &name_problem,const string &MoFEMFE_name) {
  ...
  try {
    ...
    BitFEId f_id = get_BitFEId(MoFEMFE_name);
    bool success = set.modify(miit,problem_MoFEMFE_change_bit_add(f_id));
    if(!success) SETERRQ(PETSC_COMM_SELF,1,"modification unsucceeded");
  } catch (const char* msg) {
    SETERRQ(PETSC_COMM_SELF,1,msg);
  }
...
PetscErrorCode moabField_Core::build_finite_element(const EntMoFEMFE &EntFe,int verb) {
  PetscFunctionBegin;
  ...
  for(unsigned int ii = 0;ii<BitFieldId().size();ii++) {
    ...
    for(;eit2!=adj_ents.end();eit2++) {
	  ...
	  try {
	    uid = MoFEMEntity(moab,&*miit,&*ref_ent_miit,false).get_unique_id();
	  } catch (const char* msg) {
	    SETERRQ(PETSC_COMM_SELF,1,msg);
	  }
  ...
  ierr = get_MoFEMFE_dof_uid_view(dofs_moabfield,data_view,Interface::UNION,tag_data_uids_data,tag_data_uids_size); CHKERRQ(ierr);
  ...
  if(verb>2) {
    ...
    //rows
    DofMoFEMEntity_multiIndex_uid_view MoFEMFE_row_dof_uid_view;
    ierr = p.first->get_MoFEMFE_row_dof_uid_view(dofs_moabfield,MoFEMFE_row_dof_uid_view); CHKERRQ(ierr);
    DofMoFEMEntity_multiIndex_uid_view::iterator miit_row = MoFEMFE_row_dof_uid_view.begin();
    ss << "rows dofs" << endl;
    for(;miit_row!=MoFEMFE_row_dof_uid_view.end();miit_row++) ss << **miit_row << endl;
    //cols
    DofMoFEMEntity_multiIndex_uid_view MoFEMFE_col_dof_uid_view;
    ierr = p.first->get_MoFEMFE_col_dof_uid_view(dofs_moabfield,MoFEMFE_col_dof_uid_view); CHKERRQ(ierr);
...
PetscErrorCode moabField_Core::build_finite_elements(int verb) {
  ...
  if(verb>0) {
    ...
    for(;id_MoFEMFE!=finite_elements.end();id_MoFEMFE++) {
      ...
      MoFEMFE_by_id::iterator hi_miit = MoFEMFEs.upper_bound(id_MoFEMFE->get_id());
      int count = std::distance(miit,hi_miit);
      ostringstream ss;
      ss << *id_MoFEMFE << " Nb. FEs " << count << endl;
      PetscPrintf(PETSC_COMM_WORLD,ss.str().c_str());
    }
  }
  *build_MoFEM |= 1<<1;
...
PetscErrorCode moabField_Core::build_problems(int verb) {
  ...
  for(;p_miit!=problems.end();p_miit++) {
	...
	if((miit3->get_BitRefLevel()&p_miit->get_BitRefLevel())==p_miit->get_BitRefLevel()) {
	  ierr = miit3->get_MoFEMFE_row_dof_uid_view(dofs_moabfield,dofs_rows); CHKERRQ(ierr);
	  ierr = miit3->get_MoFEMFE_col_dof_uid_view(dofs_moabfield,dofs_cols); CHKERRQ(ierr);
	}
      }
    }
    //zero rows
...
PetscErrorCode moabField_Core::partition_finite_elements(const string &name,int verb) {
  ...
  for(;miit3!=hi_miit2;miit3++) {
    ...
    if((miit3->get_id()&p_miit->get_BitFEId()).any()) {
      ...
      ierr = get_MoFEMFE_dof_uid_view(p_miit->numered_dofs_rows,rows_view,Interface::UNION,tag_row_uids_data,tag_row_uids_size); CHKERRQ(ierr);
      pair<NumeredMoFEMFE_multiIndex::iterator,bool> p = numered_finite_elements.insert(_NumeredMoFEMFE_(&*miit3));
      _NumeredMoFEMFE_ &problem_MoFEMFE = const_cast<_NumeredMoFEMFE_&>(*p.first);
      if(!p.second) {
	problem_MoFEMFE.rows_dofs.clear();
	problem_MoFEMFE.cols_dofs.clear();
      }
      bool skip = true;
	...
	parts[(*viit_rows)->part]++;
      ...
      ierr = get_MoFEMFE_dof_uid_view(p_miit->numered_dofs_cols,cols_view,Interface::UNION,tag_col_uids_data,tag_col_uids_size); CHKERRQ(ierr);
	...
	ostringstream ss;
	ss << *p_miit << endl;
	ss << problem_MoFEMFE << endl;
	typedef FENumeredDofMoFEMEntity_multiIndex::index<Unique_mi_tag>::type FENumeredDofMoFEMEntity_multiIndex_by_Unique_mi_tag;
	FENumeredDofMoFEMEntity_multiIndex_by_Unique_mi_tag::iterator miit = problem_MoFEMFE.rows_dofs.get<Unique_mi_tag>().begin();
	for(;miit!=problem_MoFEMFE.rows_dofs.get<Unique_mi_tag>().end();miit++) ss << "rows: " << *miit << endl;
	miit = problem_MoFEMFE.cols_dofs.get<Unique_mi_tag>().begin();
	for(;miit!=problem_MoFEMFE.cols_dofs.get<Unique_mi_tag>().end();miit++) ss << "cols: " << *miit << endl;
	PetscSynchronizedPrintf(PETSC_COMM_WORLD,ss.str().c_str());
      }
    }
  }
  if(verb>0) {
    typedef NumeredMoFEMFE_multiIndex::index<MoFEMFE_Part_mi_tag>::type NumeredMoFEMFE_multiIndex_by_part;
    NumeredMoFEMFE_multiIndex_by_part::iterator MoFEMFE_miit = numered_finite_elements.get<MoFEMFE_Part_mi_tag>().lower_bound(pcomm->rank());
    NumeredMoFEMFE_multiIndex_by_part::iterator hi_MoMoFEMFE_miitFEMFE_miit = numered_finite_elements.get<MoFEMFE_Part_mi_tag>().upper_bound(pcomm->rank());
    int count = std::distance(MoFEMFE_miit,hi_MoMoFEMFE_miitFEMFE_miit);
    ostringstream ss;
    ss << *p_miit;
...
PetscErrorCode moabField_Core::seed_ref_level_3D(const EntityHandle meshset,const BitRefLevel &bit) {
  ...
  try {
    ...
    for(;tit!=ents3d.end();tit++) {
      pair<RefMoFEMEntity_multiIndex::iterator,bool> p_ent = ref_entities.insert(RefMoFEMEntity(moab,*tit));
      if(!((p_ent.first->get_BitRefLevel()&bit)==bit)) {
        bool success = ref_entities.modify(p_ent.first,RefMoFEMEntity_change_add_bit(bit));
	if(!success) SETERRQ(PETSC_COMM_SELF,1,"modification unsucceeded");
      }
      pair<RefMoFEMFiniteElement_multiIndex::iterator,bool> p_MoFEMFE;
    ...
    for(int dd = 0;dd<2;dd++) {
      ...
      Range::iterator eit = ents.begin();
      for(;eit!=ents.end();eit++) {
        pair<RefMoFEMEntity_multiIndex::iterator,bool> p_ent = ref_entities.insert(RefMoFEMEntity(moab,*eit));
        if(!((p_ent.first->get_BitRefLevel()&bit)==bit)) {
	  bool success = ref_entities.modify(p_ent.first,RefMoFEMEntity_change_add_bit(bit));
	  if(!success) SETERRQ(PETSC_COMM_SELF,1,"modification unsucceeded");
        }
...
PetscErrorCode moabField_Core::seed_ref_level_MESHSET(const EntityHandle meshset,const BitRefLevel &bit) {
  PetscFunctionBegin;
  pair<RefMoFEMEntity_multiIndex::iterator,bool> p_ent = ref_entities.insert(RefMoFEMEntity(moab,meshset));
  if(!((p_ent.first->get_BitRefLevel()&bit)==bit)) {
...
PetscErrorCode moabField_Core::add_verices_in_the_middel_of_edges(const EntityHandle meshset,const BitRefLevel &bit,const bool recursive,int verb) {
  ...
  ref_ents_by_composite &ref_ents = ref_entities.get<Composite_mi_tag>();
  ...
  for(;eit!=edges.end();eit++) {
    ...
    if(miit_view == ref_parent_ents_view.end()||ref_parent_ents_view.empty()) {
      ...
      pair<RefMoFEMEntity_multiIndex::iterator,bool> p_ent = ref_entities.insert(RefMoFEMEntity(moab,node));
	...
	PetscPrintf(PETSC_COMM_WORLD,ss.str().c_str());
      ...
      bool success = ref_entities.modify(ref_entities.get<MoABEnt_mi_tag>().find(node),RefMoFEMEntity_change_add_bit(bit));
...
PetscErrorCode moabField_Core::refine_TET(const EntityHandle meshset,const BitRefLevel &bit,const bool respect_interface) {
  ...
  ref_ents_by_ent &ref_ents_ent = ref_entities.get<MoABEnt_mi_tag>();
  ...
  for(;tit!=tets.end();tit++) {
    ref_MoFEMFE_by_ent::iterator miit2 = ref_MoFEMFE.find(*tit);
    if(miit2==ref_MoFEMFE.end()) SETERRQ(PETSC_COMM_SELF,1,"this MoFEMFE is not there");
    //connectivity
    const EntityHandle* conn; 
    int num_nodes; 
    ...
    for(int nn = 0;nn<num_nodes;nn++) {
      bool success = ref_entities.modify(ref_entities.get<MoABEnt_mi_tag>().find(conn[nn]),RefMoFEMEntity_change_add_bit(bit));
      if(!success) SETERRQ(PETSC_COMM_SELF,1,"inconsitency in data");
    }
    //get edges
	...
	ref_entities.modify(tit_miit,RefMoFEMEntity_change_add_bit(bit));
	break;
      case 1:
	sub_type = 0;
    ...
    for(int tt = 0;miit_composite2!=hi_miit_composite;miit_composite2++,tt++) {
      ...
      ref_entities.modify(ref_entities.find(miit_composite2->get_ref_ent()),RefMoFEMEntity_change_add_bit(bit));
	...
	if(!ref_tets_bit.test(tt)) {
	  ...
	  pair<RefMoFEMEntity_multiIndex::iterator,bool> p_ent = ref_entities.insert(RefMoFEMEntity(moab,ref_tets[tt]));
	...
	for(;ee<6;ee++) {
	  ...
	  if(intersect(edges_nodes[ee],ref_edges_nodes).size()==2) {
	    ...
	    pair<RefMoFEMEntity_multiIndex::iterator,bool> p_ent = ref_entities.insert(RefMoFEMEntity(moab,*reit));
	    bool success = ref_entities.modify(p_ent.first,RefMoFEMEntity_change_add_bit(bit));
	    if(!success) SETERRQ(PETSC_COMM_SELF,1,"inconsitency in data");
	    if(p_ent.second) {
	      if(verbose>2) {
	...
	for(;ff<4;ff++) {
	  if(intersect(faces_nodes[ff],ref_faces_nodes).size()==3) {
	    ...
	    pair<RefMoFEMEntity_multiIndex::iterator,bool> p_ent = ref_entities.insert(RefMoFEMEntity(moab,*rfit));
	    bool success = ref_entities.modify(p_ent.first,RefMoFEMEntity_change_add_bit(bit));
	    if(!success) SETERRQ(PETSC_COMM_SELF,1,"inconsitency in data");
	    if(p_ent.second) {
	      if(verbose>2) {
...
PetscErrorCode moabField_Core::refine_PRISM(const EntityHandle meshset,const BitRefLevel &bit,int verb) {
  ...
  // find all verices which parent is edge
  typedef RefMoFEMEntity_multiIndex::index<Composite_mi_tag>::type ref_ents_by_composite;
  ref_ents_by_composite &ref_ents_by_comp = ref_entities.get<Composite_mi_tag>();
  ...
  Range::iterator pit = prisms.begin();
  for(;pit!=prisms.end();pit++) {
    ref_ENTs_by_ent::iterator miit_prism = ref_entities.get<MoABEnt_mi_tag>().find(*pit);   
    if(miit_prism==ref_entities.end()) SETERRQ(PETSC_COMM_SELF,1,"this prism is not in ref database");
    if(verb>3) {
      ostringstream ss;
    ...
    if(split_edges.count()==0) {
      ref_entities.modify(miit_prism,RefMoFEMEntity_change_add_bit(bit));
	...
	if(!ref_prism_bit.test(pp)) {
	  ...
	  pair<RefMoFEMEntity_multiIndex::iterator,bool> p_ent = ref_entities.insert(RefMoFEMEntity(moab,ref_prisms[pp]));
	  pair<RefMoFEMFiniteElement_multiIndex::iterator,bool> p_MoFEMFE = 
	    ref_finite_elements.insert(ptrWrapperRefMoFEMFiniteElement(new RefMoFEMFiniteElement_PRISM(moab,&*p_ent.first)));
...
PetscErrorCode moabField_Core::refine_MESHSET(const EntityHandle meshset,const BitRefLevel &bit,const bool recursive,int verb) {
  ...
  if(verb==-1) verb = verbose;
  typedef RefMoFEMEntity_multiIndex::index<MoABEnt_mi_tag>::type ref_ENTs_by_ent;
  ref_ENTs_by_ent::iterator miit = ref_entities.find(meshset);
  if(miit==ref_entities.end()) SETERRQ(PETSC_COMM_SELF,1,"this meshset is not in ref database");
...
PetscErrorCode moabField_Core::refine_get_ents(const BitRefLevel &bit,const EntityHandle meshset) {
  PetscFunctionBegin;
  RefMoFEMEntity_multiIndex::iterator miit = ref_entities.begin();
  for(;miit!=ref_entities.end();miit++) {
    BitRefLevel bit2 = miit->get_BitRefLevel(); 
    if((bit2&bit)==bit) {
      EntityHandle ent = miit->get_ref_ent();
      rval = moab.add_entities(meshset,&ent,1); CHKERR_PETSC(rval);
...
PetscErrorCode moabField_Core::refine_get_childern(
  ...
  ref_ents_by_composite &ref_ents = ref_entities.get<Composite_mi_tag2>();
  Range ents;
...
PetscErrorCode moabField_Core::problem_get_FE(const string &problem_name,const string &fe_name,const EntityHandle meshset) {
  ...
  NumeredMoFEMFE_multiIndex &numered_finite_elements = const_cast<NumeredMoFEMFE_multiIndex&>(p_miit->numered_finite_elements);
  NumeredMoFEMFE_multiIndex::index<MoFEMFE_name_mi_tag>::type::iterator miit = numered_finite_elements.get<MoFEMFE_name_mi_tag>().lower_bound(fe_name);
  for(;miit!=numered_finite_elements.get<MoFEMFE_name_mi_tag>().upper_bound(fe_name);miit++) {
...
PetscErrorCode moabField_Core::get_msId_3dENTS_split_sides(
  ...
  for(;eit!=ents.end();eit++) {
    ...
    switch (moab.type_from_handle(*eit)) {
	  ...
	  if(new_ents.find(*new_face.begin())==new_ents.end()) SETERRQ(PETSC_COMM_SELF,1,"database insonistency");
	  ...
	  if(new_ents.find(*new_edge.begin())==new_ents.end()) SETERRQ(PETSC_COMM_SELF,1,"database insonistency");
...
PetscErrorCode moabField_Core::add_prism_to_Adj_prisms(const EntityHandle prism,int verb) {
  ...
  try {
    pair<RefMoFEMEntity_multiIndex::iterator,bool> p_ent = ref_entities.insert(RefMoFEMEntity(moab,prism));
    pair<RefMoFEMFiniteElement_multiIndex::iterator,bool> p_MoFEMFE;
    if(p_ent.second) {
      p_MoFEMFE = ref_finite_elements.insert(ptrWrapperRefMoFEMFiniteElement(new RefMoFEMFiniteElement_PRISM(moab,&*p_ent.first)));
      SideNumber_multiIndex &side_table = p_MoFEMFE.first->get_side_number_table();
      SideNumber_multiIndex::iterator siit = side_table.begin();
	...
	if(type == MBTRI && siit->side_number == 4) {
	  face_side4 = siit->ent;
	}
      }
      if(face_side3 == no_handle) SETERRQ(PETSC_COMM_SELF,1,"database insonistency");
