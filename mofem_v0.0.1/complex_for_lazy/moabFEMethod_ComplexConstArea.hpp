...
/** 
  ...
  *
  */
struct C_CONSTANT_AREA_FEMethod: public moabField::FEMethod {
  ...
  C_CONSTANT_AREA_FEMethod(Interface& _moab,Range &_surface,Mat _C,string _lambda_field_name,int _verbose = 0):
    moab(_moab),C(_C),surface(_surface),lambda_field_name(_lambda_field_name),verbose(_verbose) { 
    diffNTRI.resize(3,2);
  ...
  PetscErrorCode calulate_CONROL_AREA(double *diffNTRI,double *dofs_X,double *dofs_iX,int sense,double *C,double *iC) {
    PetscFunctionBegin;
    ...
    ShapeFaceNormalMBTRI_complex(diffNTRI,x_dofs_X,x_normal);
    if(sense == -1) {
      for(int nn = 0;nn<3;nn++) {
	x_normal[nn].r *= -1;
	x_normal[nn].i *= -1;
    ...
    for(int ii = 0; ii<3; ii++) {
      diffX_xi[0] += dofs_X[3*ii + 0]*diffNTRI[2*ii+0];
      diffX_xi[1] += dofs_X[3*ii + 1]*diffNTRI[2*ii+0];
      diffX_xi[2] += dofs_X[3*ii + 2]*diffNTRI[2*ii+0];
      diffX_eta[0] += dofs_X[3*ii + 0]*diffNTRI[2*ii+1];
      diffX_eta[1] += dofs_X[3*ii + 1]*diffNTRI[2*ii+1];
      diffX_eta[2] += dofs_X[3*ii + 2]*diffNTRI[2*ii+1];
      if( dofs_iX == NULL ) continue;
      i_diffX_xi[0] += dofs_iX[3*ii + 0]*diffNTRI[2*ii+0];
      i_diffX_xi[1] += dofs_iX[3*ii + 1]*diffNTRI[2*ii+0];
      i_diffX_xi[2] += dofs_iX[3*ii + 2]*diffNTRI[2*ii+0];
      i_diffX_eta[0] += dofs_iX[3*ii + 0]*diffNTRI[2*ii+1];
      i_diffX_eta[1] += dofs_iX[3*ii + 1]*diffNTRI[2*ii+1];
      i_diffX_eta[2] += dofs_iX[3*ii + 2]*diffNTRI[2*ii+1];
    }
  ...
  PetscErrorCode operator()() {
    ...
    ublas::matrix<double> ELEM_C(3,9);
    for(;siit!=hi_siit;siit++) {
      EntityHandle face = siit->ent;
      if(find(surface.begin(),surface.end(),face) == surface.end()) continue;
      ierr = getData(siit); CHKERRQ(ierr);
      ierr = calulate_CONROL_AREA(&*diffNTRI.data().begin(),&*dofs_X.data().begin(),NULL,siit->sense,&*ELEM_CONSTRAIN.data().begin(),NULL); CHKERRQ(ierr);
      double area0 = norm_2(normal0);
      ublas::noalias(ELEM_C) = ublas::zero_matrix<double>(3,9);
      for(int gg = 0;gg<g_NTRI3.size1();gg++) {
	for(int nn = 0;nn<3;nn++) {
	  for(int dd = 0;dd<9;dd++) {
	    ELEM_C(nn,dd) += area0*G_TRI_W[gg]*g_NTRI3(gg,nn)*ELEM_CONSTRAIN(dd);
	  }	
	}
      }
      cerr << ELEM_CONSTRAIN << endl;
      cerr << ELEM_C << endl;
      ierr = MatSetValues(C,
        ent_global_row_indices.size(),&(ent_global_row_indices.data()[0]),
        ent_global_col_indices.size(),&(ent_global_col_indices.data()[0]),
        &*ELEM_C.data().begin(),ADD_VALUES); CHKERRQ(ierr);
    }
    PetscFunctionReturn(0);
  }
