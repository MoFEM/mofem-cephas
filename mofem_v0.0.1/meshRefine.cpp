 ...
 * --------------------------------------------------------------
 * FIXME: tet refimen should be rewriten, with better error control, and more clear refinment aptterns.
 */
...
static const int cyclic_node_rotate_face_3[3][4] = { { 3,1,0,2 }, { 0,1,2,3 }, { 2,1,3,0 }  }; // 2,0,1
...
int tet_type_5(Interface& moab,const EntityHandle *conn,const EntityHandle *edge_new_nodes,EntityHandle *new_tets_conn) {
  ...
  EntityHandle conn_[] = { conn[edges_conn[edge0*2+0]],conn[edges_conn[edge0*2+1]],conn[edges_conn[free_edge*2+0]],conn[edges_conn[free_edge*2+1]] };
...
int tet_type_4(const EntityHandle *conn,const int *split_edges,const EntityHandle *edge_new_nodes,EntityHandle *new_tets_conn) {
  char mach_pattern = 0;
  for(int ee = 0;ee<4;ee++) mach_pattern|=edge_bits_mark[split_edges[ee]];
...
int tet_type_3(const EntityHandle *conn,const int *split_edges,const EntityHandle *edge_new_nodes,EntityHandle *new_tets_conn) {
  char mach_pattern = 0;
  for(int ee = 0;ee<3;ee++) mach_pattern |= edge_bits_mark[split_edges[ee]];
  ...
  if(type == 0) {
    ...
    for(int rotate_idx = 0;rotate_idx<3;rotate_idx++) {
      //fprintf(stderr,"%d\n",rotate_idx);
      for(int ii = 0;ii<4;ii++) conn_[ii] = conn__[ cyclic_node_rotate_face_3[rotate_idx][ii] ];
      for(int ee = 0;ee<6;ee++) edge_new_nodes_[ee] = edge_new_nodes__[ cyclic_edge_rotate_face_3[rotate_idx][ee] ];
      if(( conn_[0] > conn_[2] )&&( conn_[0] > conn_[3] )) break;
    }
...
int tet_type_2(const EntityHandle *conn,const int *split_edges,const EntityHandle *edge_new_nodes,EntityHandle *new_tets_conn) {
  if(split_edges[0] == oposite_edge[split_edges[1]]) {
    ...
    int sub_type = 0;
    //type 2a
    const char mach_pattern = edge_bits_mark[split_edges[0]] | edge_bits_mark[split_edges[1]];
    EntityHandle conn_[4] = {no_handle,no_handle,no_handle,no_handle};
    int edges_[6];
    for(int ee = 0;ee<6;ee++) {
      char pattern;
...
PetscErrorCode prism_type_2(const EntityHandle *conn,const BitRefEdges split_edges,const EntityHandle *edge_new_nodes,EntityHandle *new_prism_conn) {
  ...
  for(;ee<3;ee++) {
   ...
   if(mach_pattern == split_edges) break;
  }
  if(ee>2) SETERRQ(PETSC_COMM_SELF,1,"data inonsisetncy");
