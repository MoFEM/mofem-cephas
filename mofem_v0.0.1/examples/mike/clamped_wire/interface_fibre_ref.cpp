...
int main(int argc, char *argv[]) {
    ...
    BitRefLevel general_level[mesh_refinement_level+1];
    EntityHandle meshset_level[mesh_refinement_level+1];
...
//    ierr = mField.get_msId_meshset(1,NodeSet,meshset_NodeSet1); CHKERRQ(ierr);    
    ...
    for (int ref_lev=0; ref_lev<mesh_refinement_level+1; ref_lev++ ) {
        ...
        if (ref_lev != 0) {
            ierr = mField.add_verices_in_the_middel_of_edges(meshset_level[ref_lev-1],general_level[ref_lev]); CHKERRQ(ierr);
            ierr = mField.refine_TET(meshset_level[ref_lev-1],general_level[ref_lev]); CHKERRQ(ierr);
        }

        ierr = mField.refine_get_childern(meshset_SideSet1, general_level[ref_lev],meshset_SideSet1,MBTRI,true); CHKERRQ(ierr);
        ierr = mField.refine_get_childern(meshset_SideSet2, general_level[ref_lev],meshset_SideSet2,MBTRI,true); CHKERRQ(ierr);
        ierr = mField.refine_get_childern(meshset_BlockSet1,general_level[ref_lev],meshset_BlockSet1,MBTET,true); CHKERRQ(ierr);
        ierr = mField.refine_get_childern(meshset_BlockSet2,general_level[ref_lev],meshset_BlockSet2,MBTET,true); CHKERRQ(ierr);
//        ierr = mField.refine_get_childern(meshset_SideSet5, general_level[ref_lev],meshset_SideSet5,MBTRI,true); CHKERRQ(ierr);
//        ierr = mField.refine_get_childern(meshset_SideSet6, general_level[ref_lev],meshset_SideSet6,MBTRI,true); CHKERRQ(ierr);
//        ierr = mField.refine_get_childern(meshset_NodeSet1, general_level[ref_lev],meshset_NodeSet1,MBVERTEX,true); CHKERRQ(ierr);

        rval = moab.create_meshset(MESHSET_SET,meshset_level[ref_lev]); CHKERR_PETSC(rval);
        ierr = mField.seed_ref_level_3D(0,general_level[ref_lev]); CHKERRQ(ierr);
        ierr = mField.refine_get_ents(general_level[ref_lev],meshset_level[ref_lev]); CHKERRQ(ierr);
