...
struct MyElasticFEMethod: public ArcInterfaceElasticFEMethod {
    
    Range& SideSet3;
    Range& SideSet4;
    
    MyElasticFEMethod(Interface& _moab): ArcInterfaceElasticFEMethod(_moab), SideSet3(dummy), SideSet4(dummy) {};
    ...
    ArcInterfaceElasticFEMethod(_moab,_dirihlet_ptr,_Aij,_F,_lambda,_mu,_SideSet1,_SideSet2,_arc_ptr),SideSet3(_SideSet3),SideSet4(_SideSet4)  {};
    ...
    PetscErrorCode ApplyDirihletBC() {
        ...
        DirihletBC.resize(0);
        Range::iterator siit1 = SideSet3_.begin();
        for(;siit1!=SideSet3_.end();siit1++) {
        ...
        DirihletBC.resize(0);
        Range::iterator siit2 = SideSet4_.begin();
        for(;siit2!=SideSet4_.end();siit2++) {
        ...
        /*//Set Dirihlet for SideSet1
         ...
         double fixedY=4;
         double fixedZ=5;
         EntityHandle ent_hand = riit->get_ent();
         EntityType ent_type = riit->get_ent_type();
         if (ent_type==MBVERTEX) if(riit->get_dof_rank()==0) rval=moab.tag_set_data(th_bcs, &ent_hand, 1, &fixedX);
         if(riit->get_dof_rank()==0){//0-X, 1-Y, 2-Z
         DirihletBC.push_back(riit->get_petsc_gloabl_dof_idx());
         for(int cc = 0;cc<col_mat;cc++) {
         vector<DofIdx>::iterator it = find(ColGlob[cc].begin(),ColGlob[cc].end(),riit->get_petsc_gloabl_dof_idx());
         if( it!=ColGlob[cc].end() ) *it = -1; // of idx is set -1 column is not assembled
         ...
         } //apply fix X for all entities to sideset 3
         if (ent_type==MBVERTEX) { //Apply BC to nodes
         double coords[3];
         EntityHandle node = riit->get_ent();
         moab.get_coords(&node,1,coords);
         if(riit->get_dof_rank()==1){
         if  (coords[1]<-0.4){
         if (ent_type==MBVERTEX) if(riit->get_dof_rank()==1) rval=moab.tag_set_data(th_bcs, &ent_hand, 1, &fixedY);
         DirihletBC.push_back(riit->get_petsc_gloabl_dof_idx());
         for(int cc = 0;cc<col_mat;cc++) {
         vector<DofIdx>::iterator it = find(ColGlob[cc].begin(),ColGlob[cc].end(),riit->get_petsc_gloabl_dof_idx());
         if( it!=ColGlob[cc].end() ) *it = -1; // of idx is set -1 column is not assembled
         ...
         if  (coords[2]<-0.4){
         if (ent_type==MBVERTEX) if(riit->get_dof_rank()==2) rval=moab.tag_set_data(th_bcs, &ent_hand, 1, &fixedZ);
         DirihletBC.push_back(riit->get_petsc_gloabl_dof_idx());
         for(int cc = 0;cc<col_mat;cc++) {
         vector<DofIdx>::iterator it = find(ColGlob[cc].begin(),ColGlob[cc].end(),riit->get_petsc_gloabl_dof_idx());
         if( it!=ColGlob[cc].end() ) *it = -1; // of idx is set -1 column is not assembled
         ...
         if (ent_type==MBEDGE) {//Apply BC to edges
         double coords[6];
         EntityHandle edges = riit->get_ent();
         moab.get_coords(&edges,2,coords);
         if(riit->get_dof_rank()==1){
         if  (coords[1]< -0.4 && coords[4]< -0.4){
         DirihletBC.push_back(riit->get_petsc_gloabl_dof_idx());
         for(int cc = 0;cc<col_mat;cc++) {
         vector<DofIdx>::iterator it = find(ColGlob[cc].begin(),ColGlob[cc].end(),riit->get_petsc_gloabl_dof_idx());
         if( it!=ColGlob[cc].end() ) *it = -1; // of idx is set -1 column is not assembled
...
int main(int argc, char *argv[]) {
  ...
  MyElasticFEMethod MyFE(moab,&myDirihletBC,Aij,F,LAMBDA(YoungModulus,PoissonRatio),MU(YoungModulus,PoissonRatio),SideSet1,SideSet2,SideSet3,SideSet4,ArcCtx);
  ArcInterfaceFEMethod IntMyFE(moab,&myDirihletBC,Aij,F,D,YoungModulus,h,beta,ft,Gf,SideSet1,SideSet2,ArcInterfaceFEMethod::ctx_IntLinearSoftening);
