...
//    PetscErrorCode CalcAnglesAtGaussPointsFromAnalyticalFunction(ublas::vector<double> &coords, ublas::vector<double> Angles){
//        
//        if (coords[1]==0) {
//            Angles[2]=0;
//        }
//        else{
//            Angles[2]=acos(coords[0]/coords[1]);
//        }
//    }
...
//        printf("\n");
    ...
    PetscErrorCode operator()() {
        ...
        for(int gg=0;gg<coords_at_Gauss_nodes.size();gg++){
            ...
            AngleZ=atan((coords_at_Gauss_nodes[gg]).data()[1]/(coords_at_Gauss_nodes[gg]).data()[0]);
...
//  ierr = mField.add_ents_to_finite_element_EntType_by_bit_ref(bit_level0,"ELASTIC",MBTET); CHKERRQ(ierr);
  ...
  Range SideSet1,SideSet2,NodeSet1;
...
//  };
  ...
  const double AngleX = (0.0 * M_PI)/180; //theta
  const double AngleY = (0.0 * M_PI)/180; //alpha
  const double AngleZ = (0.0 * M_PI)/180; //psi

    struct MyDirihletBC: public BaseDirihletBC {
        Range& SideSet1, NodeSet1;
        Range SideSet1_;
        
        // Constructor
        MyDirihletBC(Interface &moab,Range& _SideSet1, Range& _NodeSet1): 
        BaseDirihletBC(),SideSet1(_SideSet1),NodeSet1(_NodeSet1){
...
//            ierr = InternalClassBCSet(fe_method_ptr,RowGlob,ColGlob,DirihletBC,field_name,NodeSet1,fixed_x|fixed_y|fixed_z,false); CHKERRQ(ierr);
    ...
    MyDirihletBC myDirihletBC(moab,SideSet1,NodeSet1);
