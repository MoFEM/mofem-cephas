#!/bin/sh

while test $# -gt 0; do
        case "$1" in
                -h|--help)
                        echo "converence_study"
                        echo " "
                        echo "convergence_study [arguments]"
                        echo " "
                        echo "options:"
                        echo "-h, --help"
                        echo "-f, --file_name=FILE_NAME"
			echo "-a, --da=CRACK_AREA_INCREMENT"
			echo "-n, --nb_steps=NUMBER_OF_STEPS"
			echo "-g, --gc=GRIFFITH_ENERGY"
                        exit 0
                        ;;
                -f)
                        shift
                        if test $# -gt 0; then
                                export FILE=$1
                        else
                                echo "no file name specified"
                                exit 1
                        fi
                        shift
                        ;;
                --file_name*)
                        export FILE_NAME=`echo $1 | sed -e 's/^[^=]*=//g'`
                        shift
                        ;;
		-a)
                        shift
                        if test $# -gt 0; then
                                export DA=$1
                        else
                                echo "no crack area increment energy specified"
                                exit 1
                        fi
                        shift
                        ;;
                --da*)
                        export DA=`echo $1 | sed -e 's/^[^=]*=//g'`
                        shift
                        ;;
		-n)
                        shift
                        if test $# -gt 0; then
                                export NBSTEPS=$1
                        else
                                echo "no number of steps specified"
                                exit 1
                        fi
                        shift
                        ;;
                --nb_steps*)
                        export GC=`echo $1 | sed -e 's/^[^=]*=//g'`
                        shift
                        ;;

		-g)
                        shift
                        if test $# -gt 0; then
                                export GC=$1
                        else
                                echo "no griffith energy specified"
                                exit 1
                        fi
                        shift
                        ;;
                --gc*)
                        export GC=`echo $1 | sed -e 's/^[^=]*=//g'`
                        shift
                        ;;

                *)
                        break
                        ;;
        esac
done

if [ -z \$\{FILE+x\} ]; then 
  echo "Mesh file is unset, arc_length --help"
fi

if [ -z ${DA+x} ]; then 
  echo "Crack area increment is unset, arc_length --help"
  exit 1
fi

if [ -z ${DA+x} ]; then 
  echo "Number of steps is unset, arc_length --help"
  exit 1
fi

if [ -z ${GC+x} ]; then 
  echo "Griffith energy is unset, arc_length --help"
  exit 1
fi

DIR_SURFIX=`date "+DATE-%Y-%m-%d_TIME_%H_%M_%S"`
DIR_PREFIX=`basename $FILE .h5m`
DIR_NAME=arc_length_"$DIR_PREFIX"_"$DIR_SURFIX"

BIN_PATH="/Users/likask/MyBuild/build_mofem/v0.0.1_release_petsc_nodebug/examples/material_forces"
MPIRUN="/opt/build_for_gcc-mp-4.4/local/bin/mpirun -np 4"

mkdir $DIR_NAME
cp $FILE $DIR_NAME/
cd $DIR_NAME

$MPIRUN $BIN_PATH/arc_length_material_coupled \
  -my_file `basename $FILE` \
  -ksp_type fgmres -pc_type lu -pc_factor_mat_solver_package superlu_dist \
  -ksp_atol 1e-10 -ksp_rtol 1e-10 \
  -snes_type newtonls -snes_linesearch_type l2 \
  -snes_max_it 100 -snes_atol 1e-5 -snes_rtol 1e-5 -snes_stol 0 \
  -snes_monitor -snes_converged_reason -ksp_monitor \
  -my_penalty 1e20 -my_gc $GC -my_alpha2 1e-3 -my_alpha3 1 -my_gamma 0. \
  -my_da $DA -my_load_steps $NBSTEPS | tee log

cd ..
