...
struct ThermalElement {
	...
	MyTriFE feConvection_Rhs; //< radiation element
    MyTriFE& getLoopFeConvection_Rhs() { return feConvection_Rhs; } //< get radiation element
	
    MyTriFE feRadiation_Rhs; //< radiation element
    MyTriFE& getLoopFeRadiation_Rhs() { return feRadiation_Rhs; } //< get radiation element
    ...
    ThermalElement(
        ...
        feRhs(m_field),feLhs(m_field),feFlux(m_field),feRadiation_Rhs(m_field),feRadiationLhs(m_field),feConvection_Rhs(m_field),feConvectionRhs(m_field),feConvectionLhs(m_field),mField(m_field) {}
    ...
    struct OpRadiationLhs:public TriElementForcesAndSurcesCore::UserDataOperator {
        ...
        OpRadiationLhs(const string field_name,
            ...
            TriElementForcesAndSurcesCore::UserDataOperator(field_name),
            dAta(data),ho_geometry(_ho_geometry),useTsB(true),commonData(common_data) {}
	
        Mat *A;
        OpRadiationLhs(const string field_name,Mat *_A,
                       RadiationData &data,CommonData &common_data,bool _ho_geometry = false):
            TriElementForcesAndSurcesCore::UserDataOperator(field_name),
            dAta(data),ho_geometry(_ho_geometry),useTsB(false),A(_A),commonData(common_data) {}
    ...
    */
    struct OpRadiation_Rhs:public TriElementForcesAndSurcesCore::UserDataOperator {
        ...
        OpRadiation_Rhs(const string field_name,RadiationData &data,CommonData &common_data,bool _ho_geometry = false):
            TriElementForcesAndSurcesCore::UserDataOperator(field_name),
            dAta(data),ho_geometry(_ho_geometry),useTsF(true),commonData(common_data) {}
	
	
        Vec F;
        OpRadiation_Rhs(const string field_name,Vec _F,RadiationData &data,CommonData &common_data,bool _ho_geometry = false):
            TriElementForcesAndSurcesCore::UserDataOperator(field_name),
            dAta(data),ho_geometry(_ho_geometry),useTsF(false),F(_F),commonData(common_data) {}
	...
	/// \biref operator to calculate convection therms on body surface and assemble to rhs residual of equations(transient)
    struct OpConvection_Rhs:public TriElementForcesAndSurcesCore::UserDataOperator {
        ...
        OpConvection_Rhs(const string field_name,
						 ConvectionData &data,CommonData &common_data,bool _ho_geometry = false):
            TriElementForcesAndSurcesCore::UserDataOperator(field_name),
            dAta(data),ho_geometry(_ho_geometry),useTsF(true),commonData(common_data) {}
		
        Vec F;
        OpConvection_Rhs(const string field_name,Vec _F,
						 ConvectionData &data,CommonData &common_data,bool _ho_geometry = false):
            TriElementForcesAndSurcesCore::UserDataOperator(field_name),
            dAta(data),ho_geometry(_ho_geometry),useTsF(false),F(_F),commonData(common_data) {}
		
        ublas::vector<FieldData> Nf;
        /** \brief calculate thermal convection term in the rhs of equations
         *
         * K = intS N^T alpha T dS
         */
		PetscErrorCode doWork(             
			int side,EntityType type,DataForcesAndSurcesCore::EntData &data) {
			PetscFunctionBegin;
			...
			for(unsigned int gg = 0;gg<data.getN().size1();gg++) {
				...
				if(ho_geometry) {
					double area = norm_2(getNormals_at_GaussPt(gg));
					convectionConst = dAta.cOnvection*area*T_at_Gauss_pt;
				}   else {
					convectionConst = dAta.cOnvection*getArea()*T_at_Gauss_pt;
				}
	...
	*/
	PetscErrorCode addThermalConvectionElement(
		...
		for(_IT_CUBITMESHSETS_BY_BCDATA_TYPE_FOR_LOOP_(mField,SIDESET|CONVECTIONSET,it)) {
		 ierr = it->get_cubit_bc_data_structure(setOfFluxes[it->get_msId()].dAta); CHKERRQ(ierr);
		 rval = mField.get_moab().get_entities_by_type(it->meshset,MBTRI,setOfFluxes[it->get_msId()].tRis,true); CHKERR_PETSC(rval);
		 ierr = mField.add_ents_to_finite_element_by_TRIs(setOfFluxes[it->get_msId()].tRis,"THERMAL_CONVECTION_FE"); CHKERRQ(ierr);
		}
		*/
	...
	*/
	...
	/* \brief linear Steady convection terms in rhs and lhs
	 */
	...
	PetscErrorCode setTimeSteppingProblem(TsCtx &ts_ctx,string field_name,string rate_name,const string mesh_nodals_positions = "MESH_NODE_POSITIONS") {
		...
		
        {
            ...
            for(;sit!=setOfRadiation.end();sit++) {
                //add finite element
                feRadiation_Rhs.get_op_to_do_Rhs().push_back(new OpRadiation_Rhs(field_name,sit->second,commonData,ho_geometry));
            }
        }
		
		{
            ...
            for(;sit!=setOfConvection.end();sit++) {
                //add finite element
                feConvection_Rhs.get_op_to_do_Rhs().push_back(new OpConvection_Rhs(field_name,sit->second,commonData,ho_geometry));
            }
        }
		...
		loops_to_do_Rhs.push_back(TsCtx::loop_pair_type("THERMAL_RADIATION_FE",&feConvection_Rhs));
		loops_to_do_Rhs.push_back(TsCtx::loop_pair_type("THERMAL_RADIATION_FE",&feRadiation_Rhs));
