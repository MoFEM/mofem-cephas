...
int main(int argc, char *argv[]) {
  ...
  ierr = m_field.modify_problem_add_finite_element("P3","E1"); CHKERRQ(ierr);
  ierr = m_field.modify_problem_add_finite_element("P3","E2"); CHKERRQ(ierr);

  vector<string> blocked_problems;
  blocked_problems.push_back("P1");
  blocked_problems.push_back("P2");
  
  ierr = m_field.block_problem("P3",blocked_problems); CHKERRQ(ierr);
  ...
  PetscViewer viewer;
  ierr = PetscViewerASCIIOpen(PETSC_COMM_WORLD,"build_block_problem.txt",&viewer); CHKERRQ(ierr);
  MatView(m,viewer);
