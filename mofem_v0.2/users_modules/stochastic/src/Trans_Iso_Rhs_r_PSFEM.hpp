...
//                                        _lambda, _mu, _vf);
	  ...
	  //ComplianceMatrix(0,1) = SMat(0,1)*dI6 + SMat(0,2)*dI8;
	  //ComplianceMatrix(0,2) = SMat(0,2)*dI6 + SMat(0,1)*dI8;
	  //ComplianceMatrix(1,1) = SMat(0,0)*dI1 + (2*SMat(0,0)-2*SMat(0,1)+2*SMat(0,2))*dI3 + SMat(2,2)*dI5;
	  //ComplianceMatrix(1,2) = SMat(0,2)*dI1 + (SMat(2,2)+SMat(0,0)-2*(SMat(0,0)-SMat(0,1)))*dI3 + SMat(0,2)*dI5;
	  //ComplianceMatrix(2,2) = SMat(2,2)*dI1 + SMat(0,0)*dI5 + 2*(SMat(0,0)-SMat(0,1)+SMat(0,2))*dI3;
	  //ComplianceMatrix(3,3) = 2*(SMat(0,0)-SMat(0,1))*(dI1-2*dI3+dI5)+4*(SMat(0,0)-2*SMat(0,2)+SMat(2,2))*dI3;
	  //ComplianceMatrix(4,4) = SMat(4,4)*(dI6+dI8);
	  //ComplianceMatrix(5,5) = SMat(4,4)*(dI6+dI8);
	  //
	  ...
	  //cout<<"Transformed compliance matrix"<<ComplianceMatrix<<endl;
