...
namespace MoFEM {
  ...
  struct FE2_Rhs_rs_PSFEM: public FE2_ElasticFEMethod {
    ...
    virtual PetscErrorCode Rhs() {
      ...
      ierr = GetDataVector(first_field,D_elm_s[col_mat1]); CHKERRQ(ierr);
      ...
      for(int ee=0; ee<6; ee++) { //edges
        if(ColGlob[col_mat1].size()!=0) {
          ...
          ierr = GetDataVector(first_field,MBEDGE,D_elm_s[col_mat1],ee); CHKERRQ(ierr);
      ...
      for(int ff=0; ff<4; ff++) { //faces
        if(ColGlob[col_mat1].size()!=0) {
          ...
          ierr = GetDataVector(first_field,MBTRI,D_elm_s[col_mat1],ff); CHKERRQ(ierr);
      ...
      if(ColGlob[col_mat1].size()!=0) { // volumes
        ...
        ierr = GetDataVector(first_field,MBTET,D_elm_s[col_mat1]); CHKERRQ(ierr);
