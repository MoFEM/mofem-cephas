...
int main(int argc, char *argv[]) {
  ...
  const char* args_new[] = {"ELASTIC_FE_MACRO_REL","ELASTIC_1st_Ply", "ELASTIC_2nd_Ply",
    "ELASTIC_3rd_Ply", "ELASTIC_4th_Ply"};
  ...
  //cout<<"\n setep 1.0"<<endl;
  //ierr = Calc_RVE_Dmat.addElasticElements_Laminate(FE_Group_Name,NO_Layers,
  //                                                 "DISP_MACRO",stochastic_fields,nvars); CHKERRQ(ierr);
 ...
 //***************************************************************************
  ...
  ublas::matrix<double> Dmat_r_NUp, Dmat_r_NUz, Dmat_r_Gzp;
  ...
  //
  ublas::matrix<double> StressGP_Global;    StressGP_Global.resize(6,6);    StressGP_Global.clear();
  ublas::matrix<double> StressGP;           StressGP.resize(6,6);           StressGP.clear();
  ublas::matrix<double> StressGP_r_Em;      StressGP_r_Em.resize(6,6);      StressGP_r_Em.clear();
  ublas::matrix<double> StressGP_r_NUm;     StressGP_r_NUm.resize(6,6);     StressGP_r_NUm.clear();
  ublas::matrix<double> StressGP_r_Ep;      StressGP_r_Ep.resize(6,6);      StressGP_r_Ep.clear();
  ublas::matrix<double> StressGP_r_Ez;      StressGP_r_Ez.resize(6,6);      StressGP_r_Ez.clear();
  ublas::matrix<double> StressGP_r_NUp;     StressGP_r_NUp.resize(6,6);     StressGP_r_NUp.clear();
  ublas::matrix<double> StressGP_r_NUpz;    StressGP_r_NUpz.resize(6,6);    StressGP_r_NUpz.clear();
  ublas::matrix<double> StressGP_r_Gzp;     StressGP_r_Gzp.resize(6,6);     StressGP_r_Gzp.clear();
  ublas::matrix<double> StressGP_r_Ef;      StressGP_r_Ef.resize(6,6);      StressGP_r_Ef.clear();
  ublas::matrix<double> StressGP_r_NUf;     StressGP_r_NUf.resize(6,6);     StressGP_r_NUf.clear();
  ublas::matrix<double> StressGP_r_F;       StressGP_r_F.resize(6,6);       StressGP_r_F.clear();
  ublas::matrix<double> StressGP_r_Theta;   StressGP_r_Theta.resize(6,6);   StressGP_r_Theta.clear();
  ublas::matrix<double> StressGP_r_Theta_1; StressGP_r_Theta_1.resize(6,6); StressGP_r_Theta_1.clear();
  ublas::matrix<double> StressGP_r_Theta_2; StressGP_r_Theta_2.resize(6,6); StressGP_r_Theta_2.clear();
  ublas::matrix<double> StressGP_r_Theta_3; StressGP_r_Theta_3.resize(6,6); StressGP_r_Theta_3.clear();
  ublas::matrix<double> StressGP_r_Theta_4; StressGP_r_Theta_4.resize(6,6); StressGP_r_Theta_4.clear();
  ...
  StressGP_Global.resize(6,6); StressGP_Global.clear();
  
  
  //OpGet_RVEDmat feRhs;
  //OpGet_RVEDmat& getLoopFeRhs() { return feRhs; }
  
  //OpGet_RVEDmat ToGetFE_Ent(m_field_Macro,"DISP_MACRO");
  //ierr = m_field_Macro.loop_finite_elements("ELASTIC_PROBLEM_MACRO","ELASTIC_FE_MACRO_REL",ToGetFE_Ent); CHKERRQ(ierr);
  //ForcesAndSurcesCore(m_field_Macro).UserDataOperator("DISP_MACRO",MBTET);
  //, UserDataOperator::OPROW);
  //EntityHandle fe_ent = ToGetFE_Ent.getMoFEMFEPtr()->get_ent();
  ...
  do {
     ...
     */
    cout<<"\nThe number of variables: "<<probdata.num_vars<<endl;
    ...
    FE2_Calculation.Macro_FE_REL(m_field_Macro,m_field_RVE,nvars,nders,
                                 stochastic_fields,x,probdata.num_vars,
                                 probdata.NameVars,PlyAngle_new,NO_Layers,
                                 probdata.TimePoint);
    
    Dmat       = FE2_Calculation.REL_Dmat;       cout<<"\n\nThe Dmat at GP is "      <<Dmat<<endl;
    Dmat_r_Em  = FE2_Calculation.REL_Dmat_r_Em;  cout<<"\n\nThe Dmat_r_Em at GP is " <<Dmat_r_Em<<endl;
    Dmat_r_NUm = FE2_Calculation.REL_Dmat_r_NUm; cout<<"\n\nThe Dmat_r_NUm at GP is "<<Dmat_r_NUm<<endl;
    Dmat_r_Ep  = FE2_Calculation.REL_Dmat_r_Ep;  cout<<"\n\nThe Dmat_r_Ep at GP is " <<Dmat_r_Ep<<endl;
    Dmat_r_Ez  = FE2_Calculation.REL_Dmat_r_Ez;  cout<<"\n\nThe Dmat_r_Ez at GP is " <<Dmat_r_Ez<<endl;
    Dmat_r_NUp = FE2_Calculation.REL_Dmat_r_NUp; cout<<"\n\nThe Dmat_r_NUp at GP is "<<Dmat_r_NUp<<endl;
    Dmat_r_NUz = FE2_Calculation.REL_Dmat_r_NUz; cout<<"\n\nThe Dmat_r_NUz at GP is "<<Dmat_r_NUz<<endl;
    Dmat_r_Gzp = FE2_Calculation.REL_Dmat_r_Gzp; cout<<"\n\nThe Dmat_r_Gzp at GP is "<<Dmat_r_NUm<<endl;
    ...
    StressGP_Global.clear(); StressGP_Global = Calc_Stress.StressGP;
    StressGP.clear(); REL_Stress_Transformation(theta_angle,StressGP_Global,StressGP);
    ...
    FE2_PostProcStressForReliability_First Calc_Stress_NUpz(m_field_Macro,"DISP_MACRO","DISP_MACRO_r_NUpz",Dmat,Dmat_r_NUz);
    ...
    /*
    // with respect to Ef - fibre with isotropic material
    ...
    // with respect to NUf - fibre with isotropic material
