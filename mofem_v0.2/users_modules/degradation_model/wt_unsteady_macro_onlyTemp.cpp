...
#include <calculate_wt_onlyTemp.hpp>
...
int main(int argc, char *argv[]) {
  ...
  Calculate_wt_onlyTemp wt_elements(m_field);
...
//  std::cin >> wait;
  ...
  Calculate_wt_onlyTemp::LoadTimeSeries load_series_data(m_field,"THEMP_SERIES","CONC_SERIES");
  Calculate_wt_onlyTemp::UpdateAndControl update_velocities(m_field,ts,"Wt","Wt_RATE");
  Calculate_wt_onlyTemp::TimeSeriesMonitor monitor(m_field,"Wt_SERIES","Wt");
  ...
  for(_IT_SERIES_STEPS_BY_NAME_FOR_LOOP_(recorder_ptr,"Wt_SERIES",sit)) {
    ...
    if(pcomm->rank()==0) {
      ...
      ostringstream ss;
      ss << "Wt_onlyTime_" << sit->step_number << ".vtk";
