...
#ifndef __CALCULATE_RVE_DMAT_ISO_DISP_HPP
#define __CALCULATE_RVE_DMAT_ISO_DISP_HPP

namespace MoFEM {

  struct Calculate_RVE_Dmat_iso_Disp {
    ...
    Calculate_RVE_Dmat_iso_Disp(FieldInterface &m_field):
    feRhs(m_field),
    mField(m_field) {}
...
//            cout<<"fieldAtGaussPts[gg] "<<fieldAtGaussPts[gg] <<endl;
    ...
    struct OpCalculate_RVEDmat: public VolumeElementForcesAndSourcesCore::UserDataOperator {
      ...
      ~OpCalculate_RVEDmat(){
//        cout<<"Hello from class destructor OpCalculate_RVEDmat"<<endl;
      };
...
//          map<EntityHandle, ublas::vector<ublas::matrix<double> > > Dmat_RVE;
          ...
          if(type == MBVERTEX) {  //the doWork loop is 1+6+4+1 times but we want to loop over Guass points only onece
            ...
            for(int gg = 0;gg<nb_gauss_pts;gg++) {
//              cout<<"Hello from class constructor OpCalculate_RVEDmat"<<endl;
