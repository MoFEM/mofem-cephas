...
int main(int argc, char *argv[]) {
  ...
  //ierr = mField.add_problem("BC1_PROBLEM"); CHKERRQ(ierr); //analytical Dirichlet
  //ierr = mField.add_problem("BC2_PROBLEM"); CHKERRQ(ierr); //analytical Dirichlet
  ...
  //AnalyticalDirihletBC::DirihletBC analytical_ditihlet_bc2(mField,"imPRES",A,T,F);
...
//	  sPeed = pres_data.data.Speed;
  ...
  struct AnaliticalFunction {
	  ...
	  static double fUN_imag(double x,double y,double z) {
		  ...
		  //MoFEM::HelmholtzElement::BlockData blockData = MoFEM::HelmholtzElement::getblockdata()
		  //double wAvenumber = blockData.aNgularfreq/bolckData.sPeed;
		  const double wAvenumber = aNgularfreq/sPeed;
  ...
  //ierr = analytical_bc1.solveProblem(mField,"BC1_PROBLEM","BC1_FE",analytical_ditihlet_bc1); CHKERRQ(ierr);
  //ierr = analytical_bc2.solveProblem(mField,"BC2_PROBLEM","BC2_FE",analytical_ditihlet_bc2); CHKERRQ(ierr);
...
//	  col2[j-j1] = j;
  ...
  if(pcomm->rank()==0) {
    ...
    rval = moab.write_file("out_Incident_wave.vtk","VTK","",&out_meshset,1); CHKERR_PETSC(rval);
    rval = moab.delete_entities(&out_meshset,1); CHKERR_PETSC(rval);
  }


  //Copy the output .vtk file to desired location.
  char command[] = "cp out_Incident_wave.vtk ../../../../../mnt/home/Documents/mofem-cephas/mofem_v0.2/users_modules/helmholtz/";
