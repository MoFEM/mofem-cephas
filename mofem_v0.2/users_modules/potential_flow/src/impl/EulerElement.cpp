/** \file EulerElement.cpp
  \ingroup mofem_euler_elem
*/
...
#include <MoFEM.hpp>
#include <EulerElement.hpp>

using namespace boost::numeric;

namespace MoFEM {

PetscErrorCode EulerElement::addEulerElements(const string field_name,const string mesh_nodals_positions) {
  PetscFunctionBegin;
...
PetscErrorCode EulerElement::addThermalFluxElement(const string field_name,const string mesh_nodals_positions) {
  PetscFunctionBegin;
...
PetscErrorCode EulerElement::addThermalConvectionElement(const string field_name,const string mesh_nodals_positions) {
  PetscFunctionBegin;
...
PetscErrorCode EulerElement::addThermalRadiationElement(const string field_name,const string mesh_nodals_positions) {
  PetscFunctionBegin;
...
PetscErrorCode EulerElement::setThermalFiniteElementRhsOperators(string field_name,Vec &F) {
  PetscFunctionBegin;
...
PetscErrorCode EulerElement::setThermalFiniteElementLhsOperators(string field_name,Mat A) {
  PetscFunctionBegin;
  map<int,BlockData>::iterator sit = setOfBlocks.begin();
  for(;sit!=setOfBlocks.end();sit++) {
    //add finite elemen
...
PetscErrorCode EulerElement::setThermalFluxFiniteElementRhsOperators(string field_name,Vec &F,const string mesh_nodals_positions) {
  PetscFunctionBegin;
  ...
  for(;sit!=setOfFluxes.end();sit++) {
    //add finite element
    feFlux.getOpPtrVector().push_back(new OpHeatFlux(field_name,F,sit->second,ho_geometry));
  }
  PetscFunctionReturn(0);
}

PetscErrorCode EulerElement::setThermalConvectionFiniteElementRhsOperators(string field_name,Vec &F,const string mesh_nodals_positions) {
  PetscFunctionBegin;
  bool ho_geometry = false;
  if(mField.check_field(mesh_nodals_positions)) {
    ho_geometry = true;
  ...
  for(;sit!=setOfConvection.end();sit++) {
    ...
    feConvectionRhs.getOpPtrVector().push_back(new OpConvectionRhs(field_name,F,sit->second,commonData,ho_geometry));
  }
  PetscFunctionReturn(0);
}

PetscErrorCode EulerElement::setThermalConvectionFiniteElementLhsOperators(string field_name,Mat A,const string mesh_nodals_positions) {
  PetscFunctionBegin;
  bool ho_geometry = false;
  if(mField.check_field(mesh_nodals_positions)) {
    ho_geometry = true;
  ...
  for(;sit!=setOfConvection.end();sit++) {
    //add finite element
    feConvectionLhs.getOpPtrVector().push_back(new OpConvectionLhs(field_name,A,sit->second,ho_geometry));
  }
  PetscFunctionReturn(0);
}

PetscErrorCode EulerElement::setTimeSteppingProblem(string field_name,string rate_name,const string mesh_nodals_positions) {
  PetscFunctionBegin;
...

  {
    ...
    for(;sit!=setOfBlocks.end();sit++) {
      ...
      feLhs.getOpPtrVector().push_back(new OpHeatCapacityLsh(field_name,sit->second,commonData));
  ...
  //Flux
  {
    map<int,FluxData>::iterator sit = setOfFluxes.begin();
    for(;sit!=setOfFluxes.end();sit++) {
      feFlux.getOpPtrVector().push_back(new OpHeatFlux(field_name,sit->second,ho_geometry));
    }
  }
  ...
  // Convection
  {
    ...
    for(;sit!=setOfConvection.end();sit++) {
      feConvectionRhs.getOpPtrVector().push_back(new OpGetTriTemperatureAtGaussPts(field_name,commonData));
      feConvectionRhs.getOpPtrVector().push_back(new OpConvectionRhs(field_name,sit->second,commonData,ho_geometry));
    }
  }
  {
    map<int,ConvectionData>::iterator sit = setOfConvection.begin();
    for(;sit!=setOfConvection.end();sit++) {
      feConvectionLhs.getOpPtrVector().push_back(new OpConvectionLhs(field_name,sit->second,ho_geometry));
    }
  }
  ...
  //Radiation
  {
    ...
    for(;sit!=setOfRadiation.end();sit++) {
      ...
      feRadiationRhs.getOpPtrVector().push_back(new OpRadiationRhs(field_name,sit->second,commonData,ho_geometry));
    }
  }
  {
    map<int,RadiationData>::iterator sit = setOfRadiation.begin();
    for(;sit!=setOfRadiation.end();sit++) {
      ...
      feRadiationLhs.getOpPtrVector().push_back(new OpRadiationLhs(field_name,sit->second,commonData,ho_geometry));
    }
  }
...
PetscErrorCode EulerElement::setTimeSteppingProblem(TsCtx &ts_ctx,string field_name,string rate_name,const string mesh_nodals_positions) {
  PetscFunctionBegin;
