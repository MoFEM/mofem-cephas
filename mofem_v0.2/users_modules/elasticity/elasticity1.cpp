...
#include <ElasticFEMethod_Stress.hpp>
...
int main(int argc, char *argv[]) {
  ...
  if(m_field.check_field("TEMP")) {
    ...
    ierr = VecGhostUpdateBegin(D,INSERT_VALUES,SCATTER_FORWARD); CHKERRQ(ierr);
    ierr = VecGhostUpdateEnd(D,INSERT_VALUES,SCATTER_FORWARD); CHKERRQ(ierr);
    ierr = m_field.set_global_ghost_vector("ELASTIC_PROB",ROW,D,INSERT_VALUES,SCATTER_REVERSE); CHKERRQ(ierr);
...
//#include <ElasticFEMethod.hpp>
////#include <PostProcHookStresses.hpp>
////#include <PostProcHookStresses1.hpp>
//#include <ElasticFEMethod_Stress.hpp>
//
//using namespace boost::numeric;
//using namespace ObosleteUsersModules;
...
//  ierr = VecGhostUpdateBegin(D,INSERT_VALUES,SCATTER_FORWARD); CHKERRQ(ierr);
//  ierr = VecGhostUpdateEnd(D,INSERT_VALUES,SCATTER_FORWARD); CHKERRQ(ierr);
//  ierr = m_field.set_local_ghost_vector("ELASTIC_PROB",COL,D,INSERT_VALUES,SCATTER_REVERSE); CHKERRQ(ierr);
//
//  
//  
//  
