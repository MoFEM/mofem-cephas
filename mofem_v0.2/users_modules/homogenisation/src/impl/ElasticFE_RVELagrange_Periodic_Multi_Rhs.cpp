...
namespace MoFEM {
  ...
  PetscErrorCode ElasticFE_RVELagrange_Periodic_Multi_Rhs::postProcess() {
    PetscFunctionBegin;
  ...
  PetscErrorCode ElasticFE_RVELagrange_Periodic_Multi_Rhs::Rhs() {
    PetscFunctionBegin;
...
//      cout<< "\nD_mat[rr] =  "<<D_mat[rr]<<endl<<endl;
        ...
        ierr = VecSetValues(F1,RowGlob[ff][rr].size(),&(RowGlob[ff][rr])[0],&(f.data())[0],ADD_VALUES); CHKERRQ(ierr);
        //      cout<<"Hello "<<endl;
        //      std::string wait;
        //      std::cin >> wait;
        applied_strain.clear();
        ...
        f=prod(D_mat[rr], applied_strain);
        //      cout<<"f = "<<f<<endl;
        ierr = VecSetValues(F2,RowGlob[ff][rr].size(),&(RowGlob[ff][rr])[0],&(f.data())[0],ADD_VALUES); CHKERRQ(ierr);
        ...
        f=prod(D_mat[rr], applied_strain);
        //      cout<<"f = "<<f<<endl;
        ierr = VecSetValues(F3,RowGlob[ff][rr].size(),&(RowGlob[ff][rr])[0],&(f.data())[0],ADD_VALUES); CHKERRQ(ierr);
        ...
        if(rank_field==3){ //This poriton will execute for mechanical problem (rank=3) only
          ...
          f=prod(D_mat[rr], applied_strain);
          //      cout<<"f = "<<f<<endl;
          ierr = VecSetValues(F4,RowGlob[ff][rr].size(),&(RowGlob[ff][rr])[0],&(f.data())[0],ADD_VALUES); CHKERRQ(ierr);
          ...
          f=prod(D_mat[rr], applied_strain);
          //      cout<<"f = "<<f<<endl;
          ierr = VecSetValues(F5,RowGlob[ff][rr].size(),&(RowGlob[ff][rr])[0],&(f.data())[0],ADD_VALUES); CHKERRQ(ierr);
          ...
          f=prod(D_mat[rr], applied_strain);
          //      cout<<"f = "<<f<<endl;
          ierr = VecSetValues(F6,RowGlob[ff][rr].size(),&(RowGlob[ff][rr])[0],&(f.data())[0],ADD_VALUES); CHKERRQ(ierr);
        }
      }
    }
    PetscFunctionReturn(0);
