static char help[] ="Solves the simple ODE dy/dt=-10y, y(0)=1.";  //exact y=exp(-10t)
/*
...
int main(int argc,char **argv)
{
   ...
   */
  ierr = MatCreateSeqAIJ(PETSC_COMM_SELF,1,1,1,0,&J);CHKERRQ(ierr);
...
//  ierr = TSSetRHSFunction(ts, PETSC_NULL, RHSFunction,&user);CHKERRQ(ierr);
  ...
  ierr = TSSetIFunction(ts,NULL,TSI_FormFunction,&user); CHKERRQ(ierr);

  /*
   Set the Jacobian matrix and the function used to compute
   Jacobians.
   */
  ierr = TSSetIJacobian(ts,J,J,FormJacobian,&user); CHKERRQ(ierr);
...
//  
////  cout<<"y[0] Before = "<<y[0]<<endl;
//  f[0]=-10*y[0]; // dy/dt = -10y
////  cout<<"f[0] After =  "<<f[0]<<endl;
//
...
#undef __FUNCT__
#define __FUNCT__ "RHSFunction"
PetscErrorCode TSI_FormFunction(TS ts,PetscReal t,Vec X,Vec X_t,Vec F,void *ptr)
{
  PetscErrorCode ierr;
  PetscScalar    *y, *f, *y_t;
  ierr = VecGetArray(X,&y);CHKERRQ(ierr);
...
/*
 ...
 the preconditioner.
 */
PetscErrorCode FormJacobian(TS ts,PetscReal t,Vec X, Vec X_t, PetscReal a, Mat J,Mat B,void *ptr)
{
  PetscErrorCode ierr;
  PetscInt  row,col;
  PetscScalar one;
...
PetscErrorCode Monitor(TS ts,PetscInt step,PetscReal time,Vec u,void *ctx)
{
  ...
  PetscReal  dt;
  PetscScalar  *y;
  double exact_sol, error;
  

  exact_sol=exp(-10*time);
  ierr = TSGetTimeStep(ts,&dt);CHKERRQ(ierr);
  ierr = VecGetArray(u,&y);CHKERRQ(ierr);
  
//  cout<<"y[0] = "<<y[0]<<endl;
//  cout<<"exact_sol = "<<exact_sol<<endl;

  error=y[0]-exact_sol;
//  cout<<"error  = "<<error<<endl;

  ierr = PetscPrintf(PETSC_COMM_WORLD,"Timestep %3D:  time = %-6g Sol = %-11g, Exact-Sol = %-11g, error = %-11g\n",step,(double)time,(double)y[0],exact_sol,error);  CHKERRQ(ierr);
  
//  ierr = PetscPrintf(PETSC_COMM_WORLD,"Timestep %3D:, time = %-11g, Sol = %-11g,  Exact-Sol = %-11g\n",step,(double)time, (double)y[0], exact_sol;  CHKERRQ(ierr);
