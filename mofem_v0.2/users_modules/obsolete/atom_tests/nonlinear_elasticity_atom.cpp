...
int main(int argc, char *argv[]) {
  ...
  ierr = m_field.add_field("TEMPERATURE",H1,1); CHKERRQ(ierr);
  ierr = m_field.modify_finite_element_add_field_data("ELASTIC","TEMPERATURE"); CHKERRQ(ierr);
  ierr = m_field.add_ents_to_field_by_TETs(0,"TEMPERATURE"); CHKERRQ(ierr);
...

  {
    EntityHandle node = 0;
    double coords[3];
    for(_IT_GET_DOFS_FIELD_BY_NAME_FOR_LOOP_(m_field,"TEMPERATURE",dof_ptr)) {
      if(dof_ptr->get_ent_type()!=MBVERTEX) continue;
      EntityHandle ent = dof_ptr->get_ent();
      if(node!=ent) {
	rval = moab.get_coords(&ent,1,coords); CHKERR_PETSC(rval);
