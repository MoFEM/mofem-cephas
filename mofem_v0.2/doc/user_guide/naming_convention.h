...
users modules. Only reliable finite elements are moved to mofem finite element
library. In principle all modifications in mofem library are only for agreed and 
discussed work.

Each user module and new functionality should have associated \em atom \em test
...
Simple directory structure consist no subdirectories. e.g. elasticity. Extended (recommended)
data structure consist subdirectories, e.g. homogenisation, and follows pattern
...
Each module should have README file.
...
pull request is accepted it is committed responsibility to verify results on
