...
namespace MoFEM {
  ...
  PetscErrorCode MoistureFE_RVELagrange_Homogenized_Stress_Disp::preProcess() {
    PetscFunctionBegin;
  ...
  PetscErrorCode MoistureFE_RVELagrange_Homogenized_Stress_Disp::postProcess() {
    PetscFunctionBegin;
  ...
  PetscErrorCode MoistureFE_RVELagrange_Homogenized_Stress_Disp::Calculate_Homo_Stress() {
    PetscFunctionBegin;
    X_mat.resize(1,3);
    ...
    Stress_Homo_elem.resize(3);   Stress_Homo_elem.clear();   //homogenised stress for one element (triangle)
      ...
      switch(rr) {
        case 0:  //for nodes
          ...
          for(int nn = 0;nn<num_nodes; nn++) {
            for(_IT_GET_DOFS_FIELD_BY_NAME_AND_ENT_FOR_LOOP_(mField,"LAGRANGE_MUL_FIELD",conn[nn],iit)) {
              Lamda[rr][nn+iit->get_dof_rank()]=iit->get_FieldData();
            }
          }
        ...
        case 1:  case 2:  case 3: { //For edges
          ...
          for(int ee=0; ee<3; ee++) {
            ...
            for(_IT_GET_DOFS_FIELD_BY_NAME_AND_ENT_FOR_LOOP_(mField,"LAGRANGE_MUL_FIELD",edge,iit)) {
        ...
        case 4: //for face
          //                    cout<<"For Face"<<endl;
          for(_IT_GET_DOFS_FIELD_BY_NAME_AND_ENT_FOR_LOOP_(mField,"LAGRANGE_MUL_FIELD",fePtr->get_ent(),iit)) {
            Lamda[rr][iit->get_EntDofIdx()]=iit->get_FieldData();
          }
          //                    for(int ii=0; ii<Lamda[rr].size(); ii++) cout<<Lamda[rr][ii]<<" ";
          //                    cout<<endl;
    ...
    int Indices[3]={0, 1, 2};
    ierr = VecSetValues(Stress_Homo,3,Indices,&(Stress_Homo_elem.data())[0],ADD_VALUES); CHKERRQ(ierr);
    
    PetscFunctionReturn(0);
  }
  
  
  PetscErrorCode MoistureFE_RVELagrange_Homogenized_Stress_Disp::operator()() {
    PetscFunctionBegin;
