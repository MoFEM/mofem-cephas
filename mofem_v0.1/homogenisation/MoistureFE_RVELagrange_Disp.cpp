...
namespace MoFEM {
    ...
    MoistureFE_RVELagrange_Disp::MoistureFE_RVELagrange_Disp(
    FieldInterface& _mField,Mat &_A,Vec& _C, Vec& _F,ublas::vector<FieldData> _applied_strain):
  ElasticFE_RVELagrange_Disp(_mField,_A, _C, _F, _applied_strain){
      ...
      /*
       ...
       N=[N1  N2 N3]
       g_g_NTRI is arranged as
       [N1 N2 N3  N1 N2 N3 ....... N1 N2 N3] with size (3*Num_gauss)
       */
      
      //cout<<"g_NTRI = "<<g_NTRI.size()<<endl;
...
//      cout<<" area = "<<area<<endl;
      ...
      for(;nn<3;nn++) {
        ...
        niit = rowPtr->get<Composite_Name_And_Ent_mi_tag>().lower_bound(boost::make_tuple("LAGRANGE_MUL_FIELD",conn_face[nn]));
        hi_niit = rowPtr->get<Composite_Name_And_Ent_mi_tag>().upper_bound(boost::make_tuple("LAGRANGE_MUL_FIELD",conn_face[nn]));
        col_niit = colPtr->get<Composite_Name_And_Ent_mi_tag>().lower_bound(boost::make_tuple("CONC",conn_face[nn]));
        hi_col_niit = colPtr->get<Composite_Name_And_Ent_mi_tag>().upper_bound(boost::make_tuple("CONC",conn_face[nn]));
...
//        std::cin >> wait;
      ...
      for(;ee<3;ee++) {
        ...
        //            cout<<"edge "<<edge<<endl;
        eiit = rowPtr->get<Composite_Name_And_Ent_mi_tag>().lower_bound(boost::make_tuple("LAGRANGE_MUL_FIELD",edge));
        hi_eiit = rowPtr->get<Composite_Name_And_Ent_mi_tag>().upper_bound(boost::make_tuple("LAGRANGE_MUL_FIELD",edge));
        
        col_eiit = colPtr->get<Composite_Name_And_Ent_mi_tag>().lower_bound(boost::make_tuple("CONC",edge));
        col_hi_eiit = colPtr->get<Composite_Name_And_Ent_mi_tag>().upper_bound(boost::make_tuple("CONC",edge));
...
//                cout<<"nodes_edge  "<<nodes_edge<<endl;
            for(;gg<g_TRI_dim;gg++) {
              rowNMatrices[row_mat1][gg].resize(1,nodes_edge);
              for(int ii=0; ii<nodes_edge; ii++){
                rowNMatrices[row_mat1][gg](0,ii)=N_edge[ee][gg1]; gg1++;;
              }
            }
          }
        }
      ...
      fiit = rowPtr->get<Composite_Name_And_Ent_mi_tag>().lower_bound(boost::make_tuple("LAGRANGE_MUL_FIELD",face_tri));
      hi_fiit = rowPtr->get<Composite_Name_And_Ent_mi_tag>().upper_bound(boost::make_tuple("LAGRANGE_MUL_FIELD",face_tri));
      
      col_fiit = colPtr->get<Composite_Name_And_Ent_mi_tag>().lower_bound(boost::make_tuple("CONC",face_tri));
      col_hi_fiit = colPtr->get<Composite_Name_And_Ent_mi_tag>().upper_bound(boost::make_tuple("CONC",face_tri));
...
//        cout<<"ColGlob[row_mat].size() "<<ColGlob[row_mat].size()<<endl;
        ...
        for(;gg<g_TRI_dim;gg++) {
          rowNMatrices[row_mat][gg].resize(1,nodes_face);
          for(int ii=0; ii<nodes_face; ii++){
              rowNMatrices[row_mat][gg](0,ii)=N_face[gg1]; gg1++;
          }
        }
        row_mat++;
      }
    ...
    PetscErrorCode MoistureFE_RVELagrange_Disp::Get_H_mat() {
      PetscFunctionBegin;
...
//        cout<<"H_mat "<<H_mat[rr]<<endl;
    ...
    //Calculate the right hand side vector, i.e. f=D_max * applied_strain and assemble it into the global force vector F
     PetscErrorCode MoistureFE_RVELagrange_Disp::Rhs() {
      PetscFunctionBegin;
      X_mat.resize(1,3);
    ...
    PetscErrorCode MoistureFE_RVELagrange_Disp::operator()() {
      PetscFunctionBegin;
        cout<<"Hi from class MoistureFE_RVELagrange_Disp"<<endl;
