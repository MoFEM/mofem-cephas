 ...
 * technique for two-phase composite material consisting of matrix and inclusion
 * of isotropic material, hence it has four independent material parameters, 
 * which are Young's modulus and Poisson's ration of matrix, and Young's modulus 
 * and Poisson's ratio of fibre/inclusion, to characterize the mechanical 
...
#include "K_rs_EmEPf_PSFEMethod.hpp"
#include "K_rs_PmEPf_PSFEMethod.hpp"
...
int main(int argc, char *argv[]) {
  ...
  cout<<"TetsInBlock for matrix "<<TetsInBlock<<endl;
  ...
  cout<<"TetsInBlock for inclusion "<<TetsInBlock_stiff_inc<<endl;
  ...
  // required for solving first-order problem
  ...
  // required for first-order problem
  ...
  // Define rows/cols and element data for C1 and C1T 
  // (for lagrange multipliers to contol the rigid body motion)
  //============================================================================
  ...
  */
  ...
  ElasticFE_RVELagrange_RigidBodyTranslation MyFE_RVELagrangeRigidBodyTrans(mField,Aij,D,F,applied_strain,"DISPLACEMENT","Lagrange_mul_disp",field_rank,
"Lagrange_mul_disp_rigid_trans");
...
//  ierr = VecView(Stress_Homo,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ...
  K_r_Young_PSFEMethod my_fe_k_r_Em(mField,Aij,D,dF,LAMBDA(young_modulus,poisson_ratio),MU(young_modulus,poisson_ratio));
...
//  ierr = VecView(dD,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ...
  //    ierr = VecView(D,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ElasticFE_RVELagrange_Homogenized_Stress_Periodic MyFE_RVEHomoStressPeriodic_r_Em(mField,Aij,D,F,&RVE_volume, applied_strain, Stress_Homo_r_Em,"DISP_r_Em","Lagrange_mul_disp",field_rank);
  ierr = mField.loop_finite_elements("STOCHASIC_PROBLEM","Lagrange_elem",MyFE_RVEHomoStressPeriodic_r_Em);  CHKERRQ(ierr);
  ...
  K_r_Poisson_PSFEMethod my_fe_k_r_Pm(mField,Aij,D,dF,LAMBDA(young_modulus,poisson_ratio),MU(young_modulus,poisson_ratio));
...
//  ierr = VecView(dD,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ...
  //    ierr = VecView(D,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ElasticFE_RVELagrange_Homogenized_Stress_Periodic MyFE_RVEHomoStressPeriodic_r_Pm(mField,Aij,D,F,&RVE_volume, applied_strain, Stress_Homo_r_Pm,"DISP_r_Pm","Lagrange_mul_disp",field_rank);
  ierr = mField.loop_finite_elements("STOCHASIC_PROBLEM","Lagrange_elem",MyFE_RVEHomoStressPeriodic_r_Pm);  CHKERRQ(ierr);
  ...
  K_r_Young_PSFEMethod my_fe_k_r_Ef(mField,Aij,D,dF,LAMBDA(young_modulus,poisson_ratio),MU(young_modulus,poisson_ratio));
...
//  ierr = VecView(dD,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ...
  //    ierr = VecView(D,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ElasticFE_RVELagrange_Homogenized_Stress_Periodic MyFE_RVEHomoStressPeriodic_r_Ef(mField,Aij,D,F,&RVE_volume, applied_strain, Stress_Homo_r_Ef,"DISP_r_Ef","Lagrange_mul_disp",field_rank);
  ierr = mField.loop_finite_elements("STOCHASIC_PROBLEM","Lagrange_elem",MyFE_RVEHomoStressPeriodic_r_Ef);  CHKERRQ(ierr);
  ...
  K_r_Poisson_PSFEMethod my_fe_k_r_Pf(mField,Aij,D,dF,LAMBDA(young_modulus,poisson_ratio),MU(young_modulus,poisson_ratio));
...
//  ierr = VecView(dD,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ...
  //    ierr = VecView(D,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ElasticFE_RVELagrange_Homogenized_Stress_Periodic MyFE_RVEHomoStressPeriodic_r_Pf(mField,Aij,D,F,&RVE_volume, applied_strain, Stress_Homo_r_Pf,"DISP_r_Pf","Lagrange_mul_disp",field_rank);
  ierr = mField.loop_finite_elements("STOCHASIC_PROBLEM","Lagrange_elem",MyFE_RVEHomoStressPeriodic_r_Pf);  CHKERRQ(ierr);
  ...
  K_rs_Young_PSFEMethod my_fe_k_rs_EmEm(mField,Aij,D,ddF,LAMBDA(young_modulus,poisson_ratio),MU(young_modulus,poisson_ratio),"DISP_r_Em");
...
//  ierr = VecView(ddD,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ...
  //    ierr = VecView(D,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ElasticFE_RVELagrange_Homogenized_Stress_Periodic MyFE_RVEHomoStressPeriodic_rs_EmEm(mField,Aij,D,F,&RVE_volume, applied_strain, Stress_Homo_rs_EmEm,"DISP_rs_EmEm","Lagrange_mul_disp",field_rank);
  ierr = mField.loop_finite_elements("STOCHASIC_PROBLEM","Lagrange_elem",MyFE_RVEHomoStressPeriodic_rs_EmEm);  CHKERRQ(ierr);
  
  if(pcomm->rank()==0){
    PetscScalar    *avec_rs_EmEm;
    VecGetArray(Stress_Homo_rs_EmEm, &avec_rs_EmEm);
    
    cout<< "\nStress_Homo_rs_EmEm = \n\n";
    for(int ii=0; ii<6; ii++){
      cout.precision(15);
      cout <<*avec_rs_EmEm<<endl;
      avec_rs_EmEm++;
    }
  }
  cout<< "\n\n";
  ...
  K_rs_EP_PSFEMethod my_fe_k_rs_EmPm(mField,Aij,D,ddF,LAMBDA(young_modulus,poisson_ratio),MU(young_modulus,poisson_ratio),"DISP_r_Pm");
...
//  ierr = VecView(ddD,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ...
  //    ierr = VecView(D,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ElasticFE_RVELagrange_Homogenized_Stress_Periodic MyFE_RVEHomoStressPeriodic_rs_EmPm(mField,Aij,D,F,&RVE_volume, applied_strain, Stress_Homo_rs_EmPm,"DISP_rs_EmPm","Lagrange_mul_disp",field_rank);
  ierr = mField.loop_finite_elements("STOCHASIC_PROBLEM","Lagrange_elem",MyFE_RVEHomoStressPeriodic_rs_EmPm);  CHKERRQ(ierr);
  
  if(pcomm->rank()==0){
    PetscScalar    *avec_rs_EmPm;
    VecGetArray(Stress_Homo_rs_EmPm, &avec_rs_EmPm);
    
    cout<< "\nStress_Homo_rs_EmPm = \n\n";
    for(int ii=0; ii<6; ii++){
      cout.precision(15);
      cout <<*avec_rs_EmPm<<endl;
      avec_rs_EmPm++;
    }
  }
  cout<< "\n\n";
  ...
  K_rs_EmEPf_PSFEMethod my_fe_k_rs_EmEf(mField,Aij,D,ddF,LAMBDA(young_modulus,poisson_ratio),MU(young_modulus,poisson_ratio),"DISP_r_Ef");
...
//  ierr = VecView(ddD,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ...
  //    ierr = VecView(D,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ElasticFE_RVELagrange_Homogenized_Stress_Periodic MyFE_RVEHomoStressPeriodic_rs_EmEf(mField,Aij,D,F,&RVE_volume, applied_strain, Stress_Homo_rs_EmEf,"DISP_rs_EmEf","Lagrange_mul_disp",field_rank);
  ierr = mField.loop_finite_elements("STOCHASIC_PROBLEM","Lagrange_elem",MyFE_RVEHomoStressPeriodic_rs_EmEf);  CHKERRQ(ierr);
  ...
  K_rs_EmEPf_PSFEMethod my_fe_k_rs_EmPf(mField,Aij,D,ddF,LAMBDA(young_modulus,poisson_ratio),MU(young_modulus,poisson_ratio),"DISP_r_Pf");
...
//  ierr = VecView(ddD,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ...
  //    ierr = VecView(D,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ElasticFE_RVELagrange_Homogenized_Stress_Periodic MyFE_RVEHomoStressPeriodic_rs_EmPf(mField,Aij,D,F,&RVE_volume, applied_strain, Stress_Homo_rs_EmPf,"DISP_rs_EmPf","Lagrange_mul_disp",field_rank);
  ierr = mField.loop_finite_elements("STOCHASIC_PROBLEM","Lagrange_elem",MyFE_RVEHomoStressPeriodic_rs_EmPf);  CHKERRQ(ierr);
  ...
  K_rs_Poisson_PSFEMethod my_fe_k_rs_PmPm(mField,Aij,D,ddF,LAMBDA(young_modulus,poisson_ratio),MU(young_modulus,poisson_ratio),"DISP_r_Pm");
...
//  ierr = VecView(ddD_PP,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ...
  //    ierr = VecView(D,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ElasticFE_RVELagrange_Homogenized_Stress_Periodic MyFE_RVEHomoStressPeriodic_rs_PmPm(mField,Aij,D,F,&RVE_volume, applied_strain, Stress_Homo_rs_PmPm,"DISP_rs_PmPm","Lagrange_mul_disp",field_rank);
  ierr = mField.loop_finite_elements("STOCHASIC_PROBLEM","Lagrange_elem",MyFE_RVEHomoStressPeriodic_rs_PmPm);  CHKERRQ(ierr);
  
  if(pcomm->rank()==0){
    PetscScalar    *avec_rs_PmPm;
    VecGetArray(Stress_Homo_rs_PmPm, &avec_rs_PmPm);
    
    cout<< "\nStress_Homo_rs_PmPm = \n\n";
    for(int ii=0; ii<6; ii++){
      cout.precision(15);
      cout <<*avec_rs_PmPm<<endl;
      avec_rs_PmPm++;
    }
  }
  cout<< "\n\n";  
  ...
  K_rs_PmEPf_PSFEMethod my_fe_k_rs_PmEf(mField,Aij,D,ddF,LAMBDA(young_modulus,poisson_ratio),MU(young_modulus,poisson_ratio),"DISP_r_Ef");
...
//  ierr = VecView(ddD_PP,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ...
  //    ierr = VecView(D,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ElasticFE_RVELagrange_Homogenized_Stress_Periodic MyFE_RVEHomoStressPeriodic_rs_PmEf(mField,Aij,D,F,&RVE_volume, applied_strain, Stress_Homo_rs_PmEf,"DISP_rs_PmEf","Lagrange_mul_disp",field_rank);
  ierr = mField.loop_finite_elements("STOCHASIC_PROBLEM","Lagrange_elem",MyFE_RVEHomoStressPeriodic_rs_PmEf);  CHKERRQ(ierr);
  ...
  K_rs_PmEPf_PSFEMethod my_fe_k_rs_PmPf(mField,Aij,D,ddF,LAMBDA(young_modulus,poisson_ratio),MU(young_modulus,poisson_ratio),"DISP_r_Pf");
...
//  ierr = VecView(ddD_PP,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ...
  //    ierr = VecView(D,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ElasticFE_RVELagrange_Homogenized_Stress_Periodic MyFE_RVEHomoStressPeriodic_rs_PmPf(mField,Aij,D,F,&RVE_volume, applied_strain, Stress_Homo_rs_PmPf,"DISP_rs_PmPf","Lagrange_mul_disp",field_rank);
  ierr = mField.loop_finite_elements("STOCHASIC_PROBLEM","Lagrange_elem",MyFE_RVEHomoStressPeriodic_rs_PmPf);  CHKERRQ(ierr);
  ...
  K_rs_Young_PSFEMethod my_fe_k_rs_EfEf(mField,Aij,D,ddF,LAMBDA(young_modulus,poisson_ratio),MU(young_modulus,poisson_ratio),"DISP_r_Ef");
...
//  ierr = VecView(ddD,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ...
  //    ierr = VecView(D,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ElasticFE_RVELagrange_Homogenized_Stress_Periodic MyFE_RVEHomoStressPeriodic_rs_EfEf(mField,Aij,D,F,&RVE_volume, applied_strain, Stress_Homo_rs_EfEf,"DISP_rs_EfEf","Lagrange_mul_disp",field_rank);
  ierr = mField.loop_finite_elements("STOCHASIC_PROBLEM","Lagrange_elem",MyFE_RVEHomoStressPeriodic_rs_EfEf);  CHKERRQ(ierr);
  
  if(pcomm->rank()==0){
    PetscScalar    *avec_rs_EfEf;
    VecGetArray(Stress_Homo_rs_EfEf, &avec_rs_EfEf);
    
    cout<< "\nStress_Homo_rs_EfEf = \n\n";
    for(int ii=0; ii<6; ii++){
      cout.precision(15);
      cout <<*avec_rs_EfEf<<endl;
      avec_rs_EfEf++;
    }
  }
  cout<< "\n\n";
  ...
  K_rs_EP_PSFEMethod my_fe_k_rs_EfPf(mField,Aij,D,ddF,LAMBDA(young_modulus,poisson_ratio),MU(young_modulus,poisson_ratio),"DISP_r_Pf");
...
//  ierr = VecView(ddD,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ...
  //    ierr = VecView(D,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ElasticFE_RVELagrange_Homogenized_Stress_Periodic MyFE_RVEHomoStressPeriodic_rs_EfPf(mField,Aij,D,F,&RVE_volume, applied_strain, Stress_Homo_rs_EfPf,"DISP_rs_EfPf","Lagrange_mul_disp",field_rank);
  ierr = mField.loop_finite_elements("STOCHASIC_PROBLEM","Lagrange_elem",MyFE_RVEHomoStressPeriodic_rs_EfPf);  CHKERRQ(ierr);
  ...
  K_rs_Poisson_PSFEMethod my_fe_k_rs_PfPf(mField,Aij,D,ddF,LAMBDA(young_modulus,poisson_ratio),MU(young_modulus,poisson_ratio),"DISP_r_Pf");
...
//  ierr = VecView(ddD_PP,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ...
  //    ierr = VecView(D,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ElasticFE_RVELagrange_Homogenized_Stress_Periodic MyFE_RVEHomoStressPeriodic_rs_PfPf(mField,Aij,D,F,&RVE_volume, applied_strain, Stress_Homo_rs_PfPf,"DISP_rs_PfPf","Lagrange_mul_disp",field_rank);
  ierr = mField.loop_finite_elements("STOCHASIC_PROBLEM","Lagrange_elem",MyFE_RVEHomoStressPeriodic_rs_PfPf);  CHKERRQ(ierr);
