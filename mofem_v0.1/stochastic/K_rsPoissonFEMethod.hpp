...
#ifndef __K_RSFEMETHOD_HPP__
#define __K_RSFEMETHOD_HPP__
...
struct K_rsPoissonFEMethod: public ElasticFEMethod {
    ...
    K_rsPoissonFEMethod( FieldInterface& _mField,Mat &_Aij,Vec _X,Vec _F,double _young,double _pois):
      ElasticFEMethod( _mField,_Aij,_X,_F,0,0,"DISPLACEMENT"),young(_young),pois(_pois),ddF(_F) {
    ...
    PetscErrorCode calculateD_r(double young, double nu) {
      ...
      D_r00 = (2*young*(nu*nu + 2*nu - 2)) /pow((nu + 1),2);
      D_r01 = -(young*(2*nu*nu + 4*nu - 1))/pow((nu + 1),2);
      D_r33 = (young*(4*nu*nu + 8*nu - 5))/(2*pow((nu + 1),2));
    ...
    PetscErrorCode calculateD_rs(double young, double nu) {
      ...
      D_rs00 = 4*young/3*pow(nu + 1,3) - 8*young/(3*pow(2*nu - 1,3));
      D_rs01 = - 8*young/(3*pow(2*nu - 1,3)) - 2*young/(3*pow(nu + 1,3));
      D_rs33 = 2*young/pow(nu + 1,3);
    ...
    virtual PetscErrorCode Rhs() {
      ...
      ierr = GetDataVector("DISP_r",D_elm_r[col_mat1]); CHKERRQ(ierr);
      col_mat1++;
      ...
      for(int ee=0; ee<6; ee++) { //edges
        if(ColGlob[col_mat1].size()!=0) {
          ...
          ierr = GetDataVector("DISP_r",MBEDGE,D_elm_r[col_mat1],ee); CHKERRQ(ierr);
      ...
      for(int ff=0; ff<4; ff++) { //faces
        if(ColGlob[col_mat1].size()!=0) {
          ...
          ierr = GetDataVector("DISP_r",MBTRI,D_elm_r[col_mat1],ff); CHKERRQ(ierr);
      ...
      if(ColGlob[col_mat1].size()!=0) { // volumes
        ...
        ierr = GetDataVector("DISP_r",MBTET,D_elm_r[col_mat1]); CHKERRQ(ierr);
