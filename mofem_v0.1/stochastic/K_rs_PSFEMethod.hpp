 ...
 * --------------------------------------------------------------
 * This routine serves as a general template to construct the 2nd-order partial
 * derivative of material constitutive matrix, D_rs, the corresponding 2nd-order
 * derivative of stiffness matrix, K_rs, and the right-hand side of the 2nd-order
 * finite-element equilibrium equation, Rhs = -[K_rs][U] - 2*[K_r][U_s] for  
 * implementing perturbation-based stochastic finite element (PSFE).
 *
 * HISTORY
 *
...
#ifndef __K_RS_PSFEMETHOD_HPP__
#define __K_RS_PSFEMETHOD_HPP__
...
struct K_rs_PSFEMethod: public ElasticFEMethod {
    ...
    K_rs_PSFEMethod( FieldInterface& _mField,Mat &_Aij,Vec _X,Vec _F,double _young,double _pois,const string& _second_field):
