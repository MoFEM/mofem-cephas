...
//    ierr = mField.modify_problem_add_finite_element("ELASTIC_MECHANICS","Lagrange_elem"); CHKERRQ(ierr);
    ...
    ierr = mField.add_ents_to_finite_element_EntType_by_bit_ref(problem_bit_level,"INTERFACE",MBPRISM,5); CHKERRQ(ierr);

    
    /******************PRISMS In the bitlevel problem_bit_level******************/
    EntityHandle out_meshset_prism;
    rval = moab.create_meshset(MESHSET_SET,out_meshset_prism); CHKERR_PETSC(rval);
    ierr = mField.get_entities_by_type_and_ref_level(problem_bit_level,BitRefLevel().set(),MBPRISM,out_meshset_prism); CHKERRQ(ierr);
    rval = moab.write_file("zahur_out_prism1.vtk","VTK","",&out_meshset_prism,1); CHKERR_PETSC(rval);
