  ...
  for (int cc = 0; cc < noOfFibres; cc++) {
	...
	for(_IT_CUBITMESHSETS_BY_SET_TYPE_FOR_LOOP_(mField,SideSet,cit)) {
        ...
        if(meshset_data_int[1]==0){
            ...
            cout<<" inside meshset_data_int" <<endl;
		}
	...
	ierr = mField.get_entities_by_type_and_ref_level(bit_levels.back(),BitRefLevel().set(),MBTET,out_meshset); CHKERRQ(ierr);
	rval = moab.write_file("out_tets.vtk","VTK","",&out_meshset,1); CHKERR_PETSC(rval);
