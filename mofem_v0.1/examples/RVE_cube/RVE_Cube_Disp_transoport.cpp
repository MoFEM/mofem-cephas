...
#include "ElasticFE_RVELagrange_Disp_Transport.hpp"
...
int main(int argc, char *argv[]) {
  ...
  //Fields
  ierr = mField.add_field("CONCENTRATION",H1,1); CHKERRQ(ierr);
  ierr = mField.add_field("Lagrange_mul_conc",H1,1); CHKERRQ(ierr);
  
  //FE
  ierr = mField.add_finite_element("TRANSPORT"); CHKERRQ(ierr);
  ierr = mField.add_finite_element("Lagrange_elem"); CHKERRQ(ierr);
  
  //Define rows/cols and element data
  ierr = mField.modify_finite_element_add_field_row("TRANSPORT","CONCENTRATION"); CHKERRQ(ierr);
  ierr = mField.modify_finite_element_add_field_col("TRANSPORT","CONCENTRATION"); CHKERRQ(ierr);
  ierr = mField.modify_finite_element_add_field_data("TRANSPORT","CONCENTRATION"); CHKERRQ(ierr);
  
  //C row as Lagrange_mul_disp and col as CONCENTRATION
  ierr = mField.modify_finite_element_add_field_row("Lagrange_elem","Lagrange_mul_conc"); CHKERRQ(ierr);
  ierr = mField.modify_finite_element_add_field_col("Lagrange_elem","CONCENTRATION"); CHKERRQ(ierr);
  
  //CT col as Lagrange_mul_disp and row as CONCENTRATION
  ierr = mField.modify_finite_element_add_field_col("Lagrange_elem","Lagrange_mul_conc"); CHKERRQ(ierr);
  ierr = mField.modify_finite_element_add_field_row("Lagrange_elem","CONCENTRATION"); CHKERRQ(ierr);
  
  //As for stress we need both displacement and temprature (Lukasz)
  ierr = mField.modify_finite_element_add_field_data("Lagrange_elem","Lagrange_mul_conc"); CHKERRQ(ierr);
  ierr = mField.modify_finite_element_add_field_data("Lagrange_elem","CONCENTRATION"); CHKERRQ(ierr);
  
  
  //define problems
  ierr = mField.add_problem("TRANSPORT_STEADYSTATE"); CHKERRQ(ierr);
  
  
  //set finite elements for problem
  ierr = mField.modify_problem_add_finite_element("TRANSPORT_STEADYSTATE","TRANSPORT"); CHKERRQ(ierr);
  ierr = mField.modify_problem_add_finite_element("TRANSPORT_STEADYSTATE","Lagrange_elem"); CHKERRQ(ierr);
  
  
  //set refinment level for problem
  ierr = mField.modify_problem_ref_level_add_bit("TRANSPORT_STEADYSTATE",bit_level0); CHKERRQ(ierr);
  
  /***/
  //Declare problem
  
  //add entitities (by tets) to the field
  ierr = mField.add_ents_to_field_by_TETs(0,"CONCENTRATION",2); CHKERRQ(ierr);
  
  
  //add finite elements entities
  ierr = mField.add_ents_to_finite_element_EntType_by_bit_ref(bit_level0,"TRANSPORT",MBTET); CHKERRQ(ierr);
  Range SurfacesFaces;
  ...
  //int order = 5;
  ierr = mField.set_field_order(0,MBTET,"CONCENTRATION",order); CHKERRQ(ierr);
  ierr = mField.set_field_order(0,MBTRI,"CONCENTRATION",order); CHKERRQ(ierr);
  ierr = mField.set_field_order(0,MBEDGE,"CONCENTRATION",order); CHKERRQ(ierr);
  ierr = mField.set_field_order(0,MBVERTEX,"CONCENTRATION",1); CHKERRQ(ierr);
  ...
  //partition
  ierr = mField.partition_problem("TRANSPORT_STEADYSTATE"); CHKERRQ(ierr);
  ierr = mField.partition_finite_elements("TRANSPORT_STEADYSTATE"); CHKERRQ(ierr);
  //what are ghost nodes, see Petsc Manual
  ierr = mField.partition_ghost_dofs("TRANSPORT_STEADYSTATE"); CHKERRQ(ierr);
  ...
  Vec F,D;
  ierr = mField.VecCreateGhost("TRANSPORT_STEADYSTATE",Row,&F); CHKERRQ(ierr);
  ierr = mField.VecCreateGhost("TRANSPORT_STEADYSTATE",Col,&D); CHKERRQ(ierr);
  
  Mat Aij;
  ierr = mField.MatCreateMPIAIJWithArrays("TRANSPORT_STEADYSTATE",&Aij); CHKERRQ(ierr);

  
  CubitDisplacementDirihletBC_ZerosRowsColumns myDirihletBC(mField,"TRANSPORT_STEADYSTATE","CONCENTRATION");
  ierr = myDirihletBC.Init(); CHKERRQ(ierr);

  //Assemble F and Aij
  MoistureFEMethod MyFE(mField,&myDirihletBC,Aij,D,F);
  ElasticFE_RVELagrange_Disp_Transport MyFE_RVELagrange_Transport(mField,&myDirihletBC,Aij,D,F,applied_strain);
  ...
  ierr = mField.loop_finite_elements("TRANSPORT_STEADYSTATE","TRANSPORT",MyFE);  CHKERRQ(ierr);
  ierr = mField.loop_finite_elements("TRANSPORT_STEADYSTATE","Lagrange_elem",MyFE_RVELagrange_Transport);  CHKERRQ(ierr);
