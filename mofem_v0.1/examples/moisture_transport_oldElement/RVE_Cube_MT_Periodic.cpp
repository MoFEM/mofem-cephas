...
int main(int argc, char *argv[]) {
  ...
  ierr = mField.add_field("LAGRANGE_MUL_FIELD",H1,field_rank); CHKERRQ(ierr);  //For lagrange multipliers to control the periodic motion
  ierr = mField.add_field("LAGRANGE_MUL_FIELD_RIGID_TRANS",NOFIELD,1); CHKERRQ(ierr);  //To control the rigid body motion (3 Traslations and 3 rotations)
  ...
  //Define rows/cols and element data for MOISTURE_FE element
  ...
  //============================================================================================================
  //C1 row as LAGRANGE_MUL_FIELD and col as CONC
  ...
  //C1T col as LAGRANGE_MUL_FIELD and row as CONC
  ...
  //Adding Prisims to Element LAGRANGE_FE (to loop over these prisims)
  ...
  ierr = mField.get_entities_by_ref_level(bit_level0,BitRefLevel().set(),meshset_level0); CHKERRQ(ierr);
  //ierr = mField.add_ents_to_finite_element_EntType_by_bit_ref(bit_level0,"LAGRANGE_FE",MBPRISM); CHKERRQ(ierr);
  ierr = mField.add_ents_to_finite_element_by_PRISMs(PrismRange, "LAGRANGE_FE"); CHKERRQ(ierr);
  
  //Adding only -ve surfaces to the field LAGRANGE_MUL_FIELD (in periodic boundary conditions size of C (3M/2 x 3N))
  ...
  //ierr = mField.set_field_order(0,MBPRISM,"LAGRANGE_MUL_FIELD",order); CHKERRQ(ierr);
  ...
  ElasticFE_RVELagrange_Periodic MyFE_RVELagrangePeriodic(mField,Aij,D,F,applied_strain,"CONC","LAGRANGE_MUL_FIELD",field_rank);
  ...
  ierr = mField.loop_finite_elements("MOISTURE_PROBLEM","LAGRANGE_FE",MyFE_RVELagrangePeriodic);  CHKERRQ(ierr);
  ...
  //    ierr = VecView(D,PETSC_VIEWER_STDOUT_WORLD); CHKERRQ(ierr);
  ElasticFE_RVELagrange_Homogenized_Stress_Periodic MyFE_RVEHomoStressPeriodic(mField,Aij,D,F,&RVE_volume,applied_strain,Stress_Homo,"CONC","LAGRANGE_MUL_FIELD",field_rank);
