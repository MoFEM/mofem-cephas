...
struct mult_H1_D_H1_homogenous {

  ublas::matrix<FieldData> &B1_Nnodes_H1,&B1_diffNnodes_H1;
  vector<ublas::matrix<FieldData> > &B1_Nedges_H1;
  vector<ublas::matrix<FieldData> > &B1_Nfaces_H1;
  ublas::matrix<FieldData> &B1_Nvolume_H1;

  ublas::matrix<FieldData> &B2_Nnodes_H1,&B2_diffNnodes_H1;
  vector<ublas::matrix<FieldData> > &B2_Nedges_H1;
  vector<ublas::matrix<FieldData> > &B2_Nfaces_H1;
  ublas::matrix<FieldData> &B2_Nvolume_H1;

  ublas::matrix<FieldData> D;

  mult_H1_D_H1(
    ublas::matrix<FieldData> &_B1_Nnodes_H1,
    ublas::matrix<FieldData> &_B1_diffNnodes_H1,
    vector<ublas::matrix<FieldData> > &_B1_Nedges_H1,
    vector<ublas::matrix<FieldData> > &_B1_Nfaces_H1,
    ublas::matrix<FieldData> &_B1_Nvolume_H1,
    ublas::matrix<FieldData> _D,
    ublas::matrix<FieldData> &_B2_Nnodes_H1,
    ublas::matrix<FieldData> &_B2_diffNnodes_H1,
    vector<ublas::matrix<FieldData> > &_B2_Nedges_H1,
    vector<ublas::matrix<FieldData> > &_B2_Nfaces_H1,
    ublas::matrix<FieldData> &_B2_Nvolume_H1):
      B1_Nnodes_H1(_B1_Nnodes_H1),B1_diffNnodes_H1(_B1_diffNnodes_H1),B1_Nedges_H1(_B1_Nedges_H1),B1_Nfaces_H1(_B1_Nfaces_H1),B1_Nvolume_H1(_B1_Nvolume_H1),
      D(_D),
      B2_Nnodes_H1(_B2_Nnodes_H1),B2_diffNnodes_H1(_B2_diffNnodes_H1),B2_Nedges_H1(_B2_Nedges_H1),B2_Nfaces_H1(_B2_Nfaces_H1),B2_Nvolume_H1(_B2_Nvolume_H1) {}

  
  ublas::vector< ublas::matrix<FieldData> > H1_D_H1_nodes;
  PetscErrorCode calculate_H1_D_H1() {
    PetscFunctionBegin;

    unsigned int rank1 = D.size1();
    unsigned int rank2 = D.size2();
    ...
    ublas::matrix<FieldData> D_H1(rank1,H1.size2());
    
    for(int r2 = 0;r2 < rank2;r2++) {
      for(int r1 = 0;r1 < rank1;r1++) {
	double val = D(r1,r2);
	if(val == 0) continue;
	for(unsigned int gg = 0,gg<B1_Nnodes.size1();gg++) {
	  cblas_dscale(H1.size2(),val,
    ...
    /*for(unsigned int gg = 0,gg<B1_Nnodes.size1();gg++) {
      for(unsigned int r1 = 0;r1<rank1;r1++) {
	for(unsigned int r2 = 0;r2<rank2;r2++) {
	  if(D(r1,r2)==0) continue;
