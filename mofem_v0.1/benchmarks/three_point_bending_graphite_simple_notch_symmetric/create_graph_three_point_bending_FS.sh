...
######
# To run: ./create_graph_three_point_bending_FS.sh >/dev/null -o 1,2,3 -r 1,2
#####
rm -r Plot_files_FS
rm *.eps
mkdir Plot_files_FS
...
do
	...
	awk '/load_path:/ {print $4,-$6}' log_files_FS/log_"$order"_"$ref_level" | tee -a Plot_files_FS/load_path_CrackArea_vs_Lambda_"$order"_"$ref_level".dat
	#ent 276 is immidiately to the left of the crack, ent 279 is immediately to the right of the crack
	echo "0 0" | tee Plot_files_FS/load_path_DispY_vs_Lambda_"$order"_"$ref_level".dat && awk '/load_path_disp ent 279 dim 1/ {print -$13,-$15}' log_files_FS/log_"$order"_"$ref_level" | tee -a Plot_files_FS/load_path_DispY_vs_Lambda_"$order"_"$ref_level".dat
	done
